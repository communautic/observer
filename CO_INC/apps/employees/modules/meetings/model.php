...
class EmployeesMeetingsModel extends EmployeesModel {
	...
	function getDetails($id, $option = "") {
		...
		$array["copies"] = $this->getEmployeeTitleFromMeetingIDs(explode(",",$array['copies']),'meetings');
   ...
   function addTask($mid,$num,$sort) {
	...
	function sortItems($task) {
		foreach($task as $key => $val) {
			$q = "UPDATE " . CO_TBL_EMPLOYEES_MEETINGS_TASKS . " set sort = '$key' WHERE id='$val'";
			$result = mysql_query($q, $this->_db->connection);
		}
		return "true";
   }
   ...
   function deleteMeetingTask($id) {
	...
	function newCheckpoint($id,$date){
 	...
 	function updateCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET date = '$date', status='0' WHERE uid = '$session->uid' and app = 'employees' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function copyMeeting($pid,$mid) {
		...
		$id_new = mysql_insert_id();
		// tasks
		$qt = "INSERT INTO " . CO_TBL_EMPLOYEES_MEETINGS_TASKS . " (mid,status,title,text,sort) SELECT $id_new,status,title,text,sort FROM " . CO_TBL_EMPLOYEES_MEETINGS_TASKS . " where mid='$mid' and bin='0'";
		$resultt = mysql_query($qt, $this->_db->connection);
		// update copies of meeting
		$qc = "SELECT copies FROM " . CO_TBL_EMPLOYEES_MEETINGS . " where id='$mid'";
		...
		$copies .= $id_new;
		$qc = "UPDATE " . CO_TBL_EMPLOYEES_MEETINGS . " SET copies = '$copies' where id='$mid'";
		$resultc = mysql_query($qc, $this->_db->connection);
		
		if ($result) {
			$pro = array();
			$pro['title'] = $this->getEmployeeTitle($pid);
			$pro['titlelink'] = $this->getEmployeeTitleFromMeetingIDs(explode(",",$id_new),'meetings');
			return $pro;
		}
	}
