...
class EvalsVDocs extends Evals {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/evals/modules/$name/";
			$this->model = new EvalsVDocsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["EVAL_VDOC_ACTION_NEW"];
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("evals_vdocs",$id,$to,$subject,$body);
...
$evalsVDocs = new EvalsVDocs("vdocs");
?>