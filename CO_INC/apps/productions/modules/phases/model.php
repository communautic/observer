...
class ProductionsPhasesModel extends ProductionsModel {
	...
	function getList($id,$sort) {
		global $session;
		if($sort == 0) {
			$sortstatus = $this->getSortStatus("productions-phases-sort-status",$id);
			if(!$sortstatus) {
				...
				switch($sortstatus) {
					case "1":
				  	...
				  	break;
				  	case "3":
				  		$sortorder = $this->getSortOrder("productions-phases-sort-order",$id);
				  		if(!$sortorder) {
							$order = "order by startdate";
							$sortcur = '1';
						} else {
		...
		$perm = $this->getProductionAccess($id);
		$sql ="";
		if( $perm ==  "guest") {
			$sql = " and a.access = '1' ";
		}
		
		$q = "select a.title,a.id,a.access,a.status,a.checked_out,a.checked_out_user,(SELECT MIN(startdate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " as b WHERE b.phaseid=a.id and b.bin='0') as startdate from " . CO_TBL_PRODUCTIONS_PHASES . " as a where a.pid = '$id' and a.bin != '1' " . $sql . $order;
		
	  	$this->setSortStatus("productions-phases-sort-status",$sortcur,$id);
		...
		$qn = "select a.id,(SELECT MIN(startdate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " as b WHERE b.phaseid=a.id and b.bin='0') as startdate from " . CO_TBL_PRODUCTIONS_PHASES . " as a where a.pid = '$id' and a.bin != '1'" . $sql . " order by startdate";
	...
	function checkoutPhase($id) {
		...
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinPhase($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_PRODUCTIONS_PHASES . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getPhaseDetails($string,$field){
		...
		foreach ($users_string as &$value) {
			$q = "SELECT id,title from " . CO_TBL_PRODUCTIONS_PHASES . " where id = '$value'";
	...
	function getPhaseTitle($id){
		$q = "SELECT title from " . CO_TBL_PRODUCTIONS_PHASES . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getDependency($id){
		$q = "SELECT title FROM " . CO_TBL_PRODUCTIONS_PHASES . " where dependency = '$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getDetails($id,$num) {
		...
		$this->_documents = new ProductionsDocumentsModel();
		
		$q = "SELECT a.*,(SELECT MIN(startdate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " as b WHERE b.phaseid=a.id and b.bin='0') as startdate,(SELECT MAX(enddate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " as b WHERE b.phaseid=a.id and b.bin='0') as enddate, (SELECT startdate FROM " . CO_TBL_PRODUCTIONS . " WHERE id=a.pid) as kickoff FROM " . CO_TBL_PRODUCTIONS_PHASES . " as a where a.id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$array["perms"] = $this->getProductionAccess($array["pid"]);
		$array["canedit"] = false;
		...
		$array["productiontitle"] = $this->getProductionTitle($array['pid']);
		$array["management"] = $this->_contactsmodel->getUserListPlain($this->getProductionField($array['pid'], 'management'));
		$array["team_print"] = $this->_contactsmodel->getUserListPlain($array["team"]);
		$array["team"] = $this->_contactsmodel->getUserList($array['team'],'productionsteam', "", $array["canedit"]);
		...
		/*$perms = $this->getProductionAccess($array["pid"]);
		$array["canedit"] = false;
		...
		// get the tasks
		$task = array();
		$qt = "SELECT * FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where phaseid = '$id' and bin='0' ORDER BY startdate,status";
		...
		while($rowt = mysql_fetch_array($resultt)) {
			...
			if($tasks["dependent"] > 0) {
				$dep = $tasks["dependent"];
				$qta = "SELECT text FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where id='$dep' and bin='0'";
		...
		$sendto = $this->getSendtoDetails("productions_phases",$id);
	...
	function setDetails($id,$title,$team,$team_ct,$protocol,$documents,$phase_access,$phase_access_orig,$phase_status,$phase_status_date,$task_startdate,$task_enddate,$task_donedate,$task_id,$task_text,$task_protocol,$task,$task_cat,$task_dependent,$task_team,$task_team_ct) {
		...
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES . " set title = '$title', team='$team', team_ct = '$team_ct', protocol = '$protocol', documents='$documents', $accesssql status = '$phase_status', $sql = '$phase_status_date', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		foreach ($task_id as $key => $value) {
			...
			$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set status = '$checked_items[$key]', donedate='$donedate', dependent = '$task_dependent[$key]', text = '$task_text[$key]', protocol = '$task_protocol[$key]', startdate = '$start', enddate = '$end', team = '$task_team_i', team_ct = '$task_team_ct_i' WHERE id='$task_id[$key]'";
			$result = mysql_query($q, $this->_db->connection);
		}
	...
	function createNew($pid,$num) {
		...
		$title = $lang["PRODUCTION_PHASE_NEW"];
		
		// get some presets from production
		$q = "SELECT startdate, management, team FROM " . CO_TBL_PRODUCTIONS . " where id = '$pid'";
		$result = mysql_query($q, $this->_db->connection);
		...
		// add phase
		$q = "INSERT INTO " . CO_TBL_PRODUCTIONS_PHASES . " set title = '$title', pid='$pid', management = '$management', team='$team', access='0', status = '0', planned_date = '$now', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		
		// calculate dates to use for first task
		// 
		$q = "SELECT MAX(enddate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " WHERE pid='$pid' and bin='0'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$text = $lang["PRODUCTION_PHASE_TASK_NEW"];
		
		// add first task 1 week starting from last phase or kick off plus 1 day
		$q = "INSERT INTO " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set pid='$pid', phaseid='$id', status = '0', text = '" . $lang["PRODUCTION_PHASE_TASK_NEW"] . "', startdate = '$startdate', enddate = '$enddate'";
			$result = mysql_query($q, $this->_db->connection);
	...
	function createDuplicate($id) {
		...
		// phase
		$q = "INSERT INTO " . CO_TBL_PRODUCTIONS_PHASES . " (pid,title,protocol,team,management,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' ".$lang["GLOBAL_DUPLICAT"]."'),protocol,team,management,'$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_PRODUCTIONS_PHASES . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		// tasks
		$qt = "SELECT id,pid,dependent,cat,text,protocol,startdate,enddate FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where phaseid='$id' and bin='0' ORDER BY startdate,status";		
		$resultt = mysql_query($qt, $this->_db->connection);
		while($rowt = mysql_fetch_array($resultt)) {
			$id = $rowt["id"];
			$pid = $rowt["pid"];
			...
			$qtn = "INSERT INTO " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set pid = '$pid', phaseid = '$id_new', dependent = '$dependent', cat = '$cat', status = '0', text = '$text', protocol = '$protocol', startdate = '$startdate', enddate = '$enddate'";
		...
		$qt = "SELECT id,dependent FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where phaseid='$id_new' and bin='0'";		
		$resultt = mysql_query($qt, $this->_db->connection);
		while($rowt = mysql_fetch_array($resultt)) {
			$id = $rowt["id"];
			$dep = 0;
   ...
   function binPhase($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set bin = '1' where phaseid='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function restorePhase($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES . " set bin = '0', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set bin = '0' where phaseid='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function deletePhase($id) {
		...
		$q = "DELETE FROM co_log_sendto WHERE what='productions_phases' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where phaseid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_PRODUCTIONS_PHASES . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
	...
	function getPhaseField($id,$field) {
		$q = "SELECT $field FROM " . CO_TBL_PRODUCTIONS_PHASES . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getTaskDependencyExists($id){
		$q = "SELECT id FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where dependent = '$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function moveDependendTasks($id,$days){
		$q = "SELECT id, startdate, enddate FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where dependent='$id'";
		$result = mysql_query($q, $this->_db->connection);
		while ($row = mysql_fetch_array($result)) {
			$tid = $row["id"];
			$startdate = $this->_date->addDays($row["startdate"],$days);
			$enddate = $this->_date->addDays($row["enddate"],$days);
			$qt = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set startdate = '$startdate', enddate = '$enddate' where id='$tid'";
	...
	function addTask($pid,$phid,$date,$cat) {
		...
		switch($cat) {
			case "1": 
				$text = $lang["PRODUCTION_PHASE_MILESTONE_NEW"];
				//$team = "";
			break;
			default:
				$text = $lang["PRODUCTION_PHASE_TASK_NEW"];
		...
		$q = "INSERT INTO " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set pid='$pid', phaseid='$phid', status = '0', text = '$text', startdate = '$date', enddate = '$date', cat = '$cat'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		
		$task = array();
		$q = "SELECT * FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			foreach($row as $key => $val) {
				$tasks[$key] = $val;
	...
	function getTasksDialog($id,$field) {
		global $lang;
		$q = "SELECT pid,startdate FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		$row = mysql_fetch_assoc($result);
		$pid = $row["pid"];
		$startdate = $row["startdate"];
		...
		$result = mysql_query($q, $this->_db->connection);
		while ($row = mysql_fetch_array($result)) {
			$str .= '<a href="#" mod="productions_phases" class="insertItem" title="' . $row["text"] . '" field="' . $field . '" did="'.$row["id"].'">' . $row["text"] . '</a>';
		}
		$str .= '</div>';	
		return $str;
	}
	...
	function deleteTask($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function restorePhaseTask($id) {
		$q = "UPDATE " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " set bin = '0' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function deletePhaseTask($id) {
		global $session;
		$q = "DELETE FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getPhasesBin($id) {
		...
		$phases = "";
		$tasks = "";
		$qph ="select id, title, bin, bintime, binuser from " . CO_TBL_PRODUCTIONS_PHASES . " where pid = '$id'";
