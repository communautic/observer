...
class ProductionsControllingModel extends ProductionsModel {
	...
	function getList($id,$sort) {
		global $session, $lang;
			...
			$array["title"] = $lang["PRODUCTION_CONTROLLING_STATUS"];
			$array["itemstatus"] = "";
   ...
   function getNumTasks($id,$status = 0,$sql="") {
	...
	function getNumMeetings($id,$status = 0,$sql="") {
	   ...
	   $q = "SELECT COUNT(id) FROM " .  CO_TBL_PRODUCTIONS_MEETINGS. " WHERE pid='$id' $sql and bin='0'";
	   $result = mysql_query($q, $this->_db->connection);
	   $count = mysql_result($result,0);
	   return $count;
   }
   ...
   function getDailyStatistic() {
		global $session;
		// select all productions that are active ie startdate is < today and endfdate > today or status != 2
		$today = date("Y-m-d");
		//$str = "";
		$q = "SELECT a.id,a.status,(SELECT MIN(startdate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " as b WHERE b.pid=a.id and b.bin = '0') as startdate ,(SELECT MAX(enddate) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS . " as b WHERE b.pid=a.id and b.bin = '0') as enddate FROM " . CO_TBL_PRODUCTIONS . " as a where a.bin='0'";
		...
		while($row = mysql_fetch_array($result)) {
			if($row["startdate"] < $today && $row["status"] != '2') {
				...
				$res = $chart["real"];
				//$str .= $chart["real"];
						$qs = "INSERT INTO " . CO_TBL_PRODUCTIONS_STATISICS . " set pid = '$id', result = '$res'";
   ...
   function getChart($id, $what, $image = 1) { 
		...
		switch($what) {
			...
			case 'realisation':
				...
				$chart["tendency"] = "tendency_negative.png";
				$qt = "SELECT MAX(donedate) as dt,enddate FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS. " WHERE status='1' and pid='$id' and bin='0'";
			...
			case 'timeing':
				...
				$chart["tendency"] = "tendency_positive.png";
				$qt = "SELECT COUNT(id) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS. " WHERE status='0' and startdate < '$today' and enddate >= '$today' and pid='$id' and bin='0'";
				$resultt = mysql_query($qt, $this->_db->connection);
				$tasks_active = mysql_result($resultt,0);
				
				$qo = "SELECT COUNT(id) FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS. " WHERE status='0' and enddate < '$today' and pid='$id' and bin='0'";
			...
			case 'tasks':
				...
				$chart["tendency"] = "tendency_positive.png";
				$qt = "SELECT status,donedate,enddate FROM " . CO_TBL_PRODUCTIONS_PHASES_TASKS. " WHERE enddate < '$today' and pid='$id' and bin='0' ORDER BY enddate DESC LIMIT 0,1";
				$resultt = mysql_query($qt, $this->_db->connection);
			...
			case 'status':
				...
				// all
				$q = "SELECT id FROM " . CO_TBL_PRODUCTIONS_PHASES. " WHERE pid = '$id' and bin = '0'";
				$result = mysql_query($q, $this->_db->connection);
				$all = mysql_num_rows($result);
				
				// planned
				$q = "SELECT id FROM " . CO_TBL_PRODUCTIONS_PHASES. " WHERE pid = '$id' and status = '0' and bin = '0'";
				$result = mysql_query($q, $this->_db->connection);
				...
				// inprogress
				$q = "SELECT id FROM " . CO_TBL_PRODUCTIONS_PHASES. " WHERE pid = '$id' and status = '1' and bin = '0'";
				$result = mysql_query($q, $this->_db->connection);
				...
				// finished
				$q = "SELECT id FROM " . CO_TBL_PRODUCTIONS_PHASES. " WHERE pid = '$id' and status = '2' and bin = '0'";
				$result = mysql_query($q, $this->_db->connection);
				...
				$chart["img_name"] = 'productions_controlling_' . $id . "_status.png";
				if($all == 0) {
...
$productionsControllingModel = new ProductionsControllingModel();
?>