...
class PatientsMeetingsModel extends PatientsModel {
   ...
   function deleteMeetingTask($id) {
	...
	function copyMeeting($pid,$mid) {
		global $session, $lang;
		// meeting
		$q = "INSERT INTO " . CO_TBL_PATIENTS_MEETINGS . " (pid,title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,documents,access,access_date,access_user,created_date,created_user,edited_date,edited_user) SELECT '$pid',title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,documents,access,access_date,access_user,created_date,created_user,edited_date,edited_user FROM " . CO_TBL_PATIENTS_MEETINGS . " where id='$mid'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		// tasks
