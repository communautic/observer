<div class="content nohead">
    <div><span class="co-link actionSendToOption" rel="1"><?php echo $lang["PATIENT_INVOICE_PRINTOPTION_INVOICE"];?></span></div>
    <?php if(CO_PRODUCT_VARIANT == 2) { ?>
    <div><span class="co-link actionSendToOption" rel="5"><?php echo $lang["PATIENT_INVOICE_PRINTOPTION_INVOICE_PLAIN"];?></span></div>
    <?php } ?>
    <div><span class="co-link actionSendToOption" rel="4"><?php echo $lang["PATIENT_INVOICE_PRINTOPTION_SERVICES"];?></span></div>
    <div><span class="co-link actionSendToOption" rel="2"><?php echo $lang["PATIENT_INVOICE_PRINTOPTION_PAYMENT_REMINDER"];?></span></div>
</div>
<span class="arrow"></span>