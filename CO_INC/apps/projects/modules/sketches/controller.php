...
class ProjectsSketches extends Projects {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/projects/modules/$name/";
			$this->model = new ProjectsSketchesModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["PROJECT_SKETCH_ACTION_NEW"];
	...
	function printDetails($id,$t,$option) {
		...
		switch($option) {
			case 'plan':
				...
				$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["PROJECT_PRINT_SKETCH"];
				switch($t) {
					case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("projects_sketches",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["PROJECT_SKETCH_HELP"];
		$data["app"] = "projects";
