...
class ProjectsDocumentsModel extends ProjectsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("projects-documents-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("projects-documents-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by title";
								$sortcur = '1';
						  } else {
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_PROJECTS_DOCUMENTS_FOLDERS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
   ...
   function setDetails($id,$title,$protocol,$document_access) {
		...
		$q = "UPDATE " . CO_TBL_PROJECTS_DOCUMENTS_FOLDERS . " set title = '$title', protocol = '$protocol', access='$document_access', access_date='$access_date', access_user = '$session->uid', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function createNew($id) {
	 ...
	 function createDuplicate($id) {
		...
		// documents
		$q = "SELECT filename,tempname,filesize FROM " . CO_TBL_PROJECTS_DOCUMENTS . " where did='$id' and bin='0'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			...
			$qt = "INSERT INTO " . CO_TBL_PROJECTS_DOCUMENTS . " set did='$id_new', filename='$fsave', filesize='$filesize'";
			$resultt = mysql_query($qt, $this->_db->connection);
		}
   ...
   function restoreDocument($id) {
	...
	function deleteDocument($id) {
		...
		$q = "DELETE FROM co_log_sendto WHERE what='projects_documents' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function deleteFile($id) {
		$q = "SELECT filename,tempname FROM " . CO_TBL_PROJECTS_DOCUMENTS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
