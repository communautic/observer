   ...
   */
   function setBrainstormDetails($id,$title) {
	...
	function deleteBrainstorm($id) {
		...
		if(in_array("phases",$active_modules)) {
			...
			$q = "SELECT id FROM co_brainstorms_phases where pid = '$id'";
			$result = mysql_query($q, $this->_db->connection);
			while($row = mysql_fetch_array($result)) {
				$pid = $row["id"];
				$brainstormsPhasesModel->deletePhase($pid);
			}
		}
   ...
   function emptyBin() {
	...
	function getEditPerms($id) {
		global $session;
		$perms = array();
		$q = "SELECT pid FROM co_brainstorms_access where admins REGEXP '[[:<:]]" . $id . "[[:>:]]'";
      	$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$perms[] = $row["pid"];
		}
   ...
   function getViewPerms($id) {
		global $session;
		$perms = array();
		$q = "SELECT pid FROM co_brainstorms_access where guests REGEXP '[[:<:]]" . $id. "[[:>:]]'";
      	$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$perms[] = $row["pid"];
		}
