...
class BrainstormsMeetingsModel extends BrainstormsModel {
	...
	function getDetails($id, $option = "") {
		...
		switch($array["status"]) {
			case "0":
				$array["status_text"] = $lang["BRAINSTORM_MEETING_STATUS_PLANNED"];
				$array["status_date"] = '';
			break;
			case "1":
				$array["status_text"] = $lang["BRAINSTORM_MEETING_STATUS_ON_SCHEDULE"];
				$array["status_date"] = '';
			break;
			case "2":
				$array["status_text"] = $lang["BRAINSTORM_MEETING_STATUS_CANCELLED"];
				$array["status_date"] = '';
				break;
			case "3":
				$array["status_text"] = $lang["BRAINSTORM_MEETING_STATUS_POSPONED"];
   ...
   function setDetails($pid,$id,$title,$meetingdate,$start,$end,$location,$location_ct,$participants,$participants_ct,$management,$management_ct,$task_id,$task_title,$task_text,$task,$task_sort,$documents,$meeting_access,$meeting_access_orig,$meeting_status,$meeting_status_date) {
		...
		$title_add = "";
		if($meeting_status == 3) {
			$title_add = " " . $lang["BRAINSTORM_MEETING_POSPONED"];
		}
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_BRAINSTORMS_MEETINGS . " set title = '" . $lang["BRAINSTORM_MEETING_NEW"] . "', item_date='$now', start='$time', end='$time', pid = '$id', participants = '$session->uid', management = '$session->uid', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
