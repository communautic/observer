...
class Controller extends MySQLDB {
	...
	function printPrescription($title,$text,$stationary=1,$headerimg='logo_print.png') {
		global $lang;
		//$header = "";
		$headerpdf = "";
		//$GLOBALS['STATIONARY'] = $stationary;
		...
		ob_start();
			include(CO_INC . "/view/prescriptionheader.php");
			$header = ob_get_contents();
		ob_end_clean();
		
		$pdfheader = CO_PATH_BASE . "/data/templates/pdf_prescription.php";
		if(file_exists($pdfheader)) {
			ob_start();
				include_once($pdfheader);
				$headerpdf = ob_get_contents();
		...
		$dompdf->load_html($html);
		//$dompdf->set_paper('a4', 'portrait'); // change 'a4' to whatever you want 
		$dompdf->set_paper(array(0, 0, 280, 594), 'portrait');
		$dompdf->render();
		$options['Attachment'] = 1;
		$options['Accept-Ranges'] = 0;
		$options['compress'] = 1;
