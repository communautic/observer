...
class ForumsVDocs extends Forums {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/forums/modules/$name/";
			$this->model = new ForumsVDocsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["FORUM_VDOC_ACTION_NEW"];
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("forums_vdocs",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["FORUM_VDOC_HELP"];
		$data["app"] = "forums";
...
$forumsVDocs = new ForumsVDocs("vdocs");
?>