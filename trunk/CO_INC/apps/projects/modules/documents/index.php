...
if (!empty($_GET['request'])) {
	switch ($_GET['request']) {
		...
		case 'getListArchive':
			$sort = "0";
			echo($projectsDocuments->getListArchive($_GET['id'],$sort));
		break;
		case 'getDetailsArchive':
			echo($projectsDocuments->getDetailsArchive($_GET['id']));
		break;
	}
}
...
if (!empty($_POST['request'])) {
	switch ($_POST['request']) {
		case 'setDetails':
			echo($projectsDocuments->setDetails($_POST['id'], $system->checkMagicQuotes($_POST['title']), $system->checkMagicQuotes($_POST['protocol']), $_POST['document_access']));
		break;
		case 'sendDetails':
