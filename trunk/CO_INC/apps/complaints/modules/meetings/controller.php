...
class ComplaintsMeetings extends Complaints {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/complaints/modules/$name/";
			$this->model = new ComplaintsMeetingsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["COMPLAINT_MEETING_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["COMPLAINT_PRINT_MEETING"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("complaints_meetings",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["COMPLAINT_MEETING_HELP"];
		$data["app"] = "complaints";
