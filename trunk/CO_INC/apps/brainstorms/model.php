...
class BrainstormsModel extends Model {
   ...
   */   
   function getNumBrainstorms($id) {
	...
	function getBrainstormTitleFromMeetingIDs($array,$target, $link = 0){
		$total = sizeof($array);
		$data = '';
		if($total == 0) { 
			return $data; 
   ...
   */
   ...
   function getWidgetAlerts() {
	...
	function getBrainstormsSearch($term,$exclude){
		...
		$q = "SELECT a.id,a.title as label FROM " . CO_TBL_BRAINSTORMS . " as a WHERE a.id != '$exclude' and a.title like '%$term%' and  a.bin='0'" . $access ."ORDER BY a.title";
	...
	function getBrainstormArray($string){
		$string = explode(",", $string);
		$total = sizeof($string);
		$items = '';
	...
	function getLast10Brainstorms() {
		global $session;
		$brainstorms = $this->getBrainstormArray($this->getUserSetting("last-used-brainstorms"));
	  return $brainstorms;
	}
	...
	function saveLastUsedBrainstorms($id) {
		global $session;
		$string = $id . "," .$this->getUserSetting("last-used-brainstorms");
		$string = rtrim($string, ",");
		...
		$this->setUserSetting("last-used-brainstorms",$str);
	  return true;
	}
