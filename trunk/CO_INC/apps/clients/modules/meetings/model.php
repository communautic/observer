...
class ClientsMeetingsModel extends ClientsModel {
	...
	function getDetails($id, $option = "") {
		...
		$array["copies"] = $this->getClientTitleFromMeetingIDs(explode(",",$array['copies']),'meetings');
		...
		$q = "SELECT date,note FROM " . CO_TBL_USERS_CHECKPOINTS . " where uid='$session->uid' and app = 'clients' and module = 'meetings' and app_id = '$id' LIMIT 1";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function setDetails($pid,$id,$title,$meetingdate,$start,$end,$location,$location_ct,$participants,$participants_ct,$management,$management_ct,$task_id,$task_title,$task_text,$task,$task_sort,$documents,$meeting_access,$meeting_access_orig) {
		...
		foreach ($task_id as $key => $value) {
			...
			$q = "UPDATE " . CO_TBL_CLIENTS_MEETINGS_TASKS . " set status = '$checked_items[$key]', title = '$task_title[$key]', text = '$task_text[$key]' WHERE id='$task_id[$key]'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return $id;
   ...
   function updateStatus($id,$date,$status) {
		...
		$q = "SELECT title FROM " . CO_TBL_CLIENTS_MEETINGS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$title = mysql_result($result,0);
		
		$title_change = $title;
		if($status == 3) {
			$title_change = $title . " " . $lang["CLIENT_MEETING_POSPONED"];
		}
		
		$q = "UPDATE " . CO_TBL_CLIENTS_MEETINGS . " set title = '$title_change', status = '$status', status_date = '$date', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		if($status == 3) {
			...
			$q = "INSERT INTO " . CO_TBL_CLIENTS_MEETINGS . " (pid,title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,created_date,created_user,edited_date,edited_user) SELECT pid,'$title','$date',start,end,location,location_ct,length,management,management_ct,participants,participants_ct,0,'$now','$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_CLIENTS_MEETINGS . " where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
			if ($result) {
				$nid = mysql_insert_id();
				// do tasks
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_CLIENTS_MEETINGS . " set title = '" . $lang["CLIENT_MEETING_NEW"] . "', item_date='$now', start='$time', end='$time', pid = '$id', participants = '$session->uid', management = '$session->uid', status = '0', status_date = '$now', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   ...
   function addTask($mid,$num,$sort) {
	...
	function sortItems($task) {
		foreach($task as $key => $val) {
			$q = "UPDATE " . CO_TBL_CLIENTS_MEETINGS_TASKS . " set sort = '$key' WHERE id='$val'";
			$result = mysql_query($q, $this->_db->connection);
		}
		return "true";
   }
   ...
   function deleteMeetingTask($id) {
	...
	function newCheckpoint($id,$date){
 	...
 	function updateCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET date = '$date', status='0' WHERE uid = '$session->uid' and app = 'clients' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function updateCheckpointText($id,$text){
		global $session;
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET note = '$text' WHERE uid = '$session->uid' and app = 'clients' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function copyMeeting($pid,$mid) {
		...
		$now = gmdate("Y-m-d H:i:s");
		// meeting
		$q = "INSERT INTO " . CO_TBL_CLIENTS_MEETINGS . " (pid,title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,documents,access,access_date,access_user,created_date,created_user,edited_date,edited_user) SELECT '$pid',title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,documents,access,access_date,access_user,'$now','$session->uid',edited_date,edited_user FROM " . CO_TBL_CLIENTS_MEETINGS . " where id='$mid'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		// tasks
		$qt = "INSERT INTO " . CO_TBL_CLIENTS_MEETINGS_TASKS . " (mid,status,title,text,sort) SELECT $id_new,status,title,text,sort FROM " . CO_TBL_CLIENTS_MEETINGS_TASKS . " where mid='$mid' and bin='0'";
		$resultt = mysql_query($qt, $this->_db->connection);
		// update copies of meeting
		$qc = "SELECT copies FROM " . CO_TBL_CLIENTS_MEETINGS . " where id='$mid'";
		...
		$copies .= $id_new;
		$qc = "UPDATE " . CO_TBL_CLIENTS_MEETINGS . " SET copies = '$copies' where id='$mid'";
		$resultc = mysql_query($qc, $this->_db->connection);
		
		if ($result) {
			$pro = array();
			$pro['title'] = $this->getClientTitle($pid);
			$pro['titlelink'] = $this->getClientTitleFromMeetingIDs(explode(",",$id_new),'meetings');
			return $pro;
		}
	}
