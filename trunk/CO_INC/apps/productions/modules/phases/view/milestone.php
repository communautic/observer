...
<table border="0" cellpadding="0" cellspacing="0" class="table-content tbl-inactive" style="margin-bottom: 0px;">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td width="12"></td>
      <td class="tcell-left text11"><span class="<?php if($phase->canedit) { ?>content-nav ui-datepicker-trigger-action<?php } ?>"><span><?php echo $lang["PRODUCTION_PHASE_MILESTONE_DATE"];?></span></span></td>
    ...
    <tr>
      ...
      <td class="tcell-right"><input id="dependent<?php echo $value->id;?>" name="task_dependent[<?php echo $value->id;?>]" type="hidden" value="<?php echo $value->dependent;?>"><span id="dependent<?php echo $value->id;?>-text"><?php if($phase->canedit) { ?><a field="dependent<?php echo $value->id;?>" uid="<?php echo $value->id;?>" class="showItemContext" href="productions_phases"><?php } ?><?php echo $value->dependent_title;?><?php if($phase->canedit) { ?></a><?php } ?></span></td>
    </tr>
  </table>
  <table width="100%" border="0" cellpadding="0" cellspacing="0" id="donedate_<?php echo $value->id;?>" style="<?php echo $donedate_field;?>">
    <tr>
      <td width="12">&nbsp;</td>
      <td class="tcell-left text11"><span class="<?php if($phase->canedit) { ?>content-nav ui-datepicker-trigger-action<?php } ?>"><span><?php echo $lang["PRODUCTION_PHASE_TASK_STATUS_FINISHED"];?></span></span></td>
