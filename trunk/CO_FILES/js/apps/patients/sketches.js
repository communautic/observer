/* sketches Object */
function patientsSketches(name) {
	this.name = name;
	var self = this;
	...
	this.createUploader = function(ele){            
		...
		var uploader = new qq.FileUploader({
			...
			onSubmit: function(id, fileName){ 
				$('#patients-right .ui-upload-list').show();
				//$('#avatarBinItem').hide();
			},
			...
			onComplete: function(id, fileName, data){
				...
				//$("#patients1 .active-link:eq(0)").trigger("click");
				//alert(data.filename + ' ' + data.id);
				
				var id = $('#patients').data('second');
				$.ajax({ type: "GET", url: "/", dataType: 'json', data: 'path=apps/patients/modules/sketches&request=createNew&id=' + data.id + '&type=2&image='+data.filename, cache: false, success: function(data){
					$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=getList&id="+id, success: function(list){
						$("#patients3 ul[rel=sketches]").html(list.html);
						$('#patients_sketches_items').html(list.items);
						var liindex = $("#patients3 ul[rel=sketches] .module-click").index($("#patients3 ul[rel=sketches] .module-click[rel='"+data.id+"']"));
						$("#patients3 ul[rel=sketches] .module-click:eq("+liindex+")").addClass('active-link');
						var moduleidx = $("#patients3 ul").index($("#patients3 ul[rel=sketches]"));
			...
			onCancel: function(id, fileName){
				$('#patients-right .ui-upload-list').hide();
				//$('#avatarBinItem').show();
				},
			debug: false
		});
	...
	this.formProcess = function(formData, form, poformOptions) {
		...
		$("#patientssketchtasks > div").each(function() {
			var id = $(this).attr('id');
			var reg = /[0-9]+/.exec(id);
		...
		/*$('#patients div.sketches_task_team_list').each(function() {
			var id = $(this).attr("id");
			var reg = /[0-9]+/.exec(id);
			formData[formData.length] = processListArray('sketches_task_team',reg);
		});*/
		
		/*$('#patients div.sketches_task_team_list_ct').each(function() {
			var id = $(this).attr("id");
			//var reg = /[0-9]+/.exec(id);
			formData[formData.length] = processCustomTextFieldArray(id);
		});*/
		
		$('#patients div.task_sketchtype').each(function() {
			var id = $(this).attr("id");
			var reg = /[0-9]+/.exec(id);
	 ...
	 this.formResponse = function(data) {
		...
		$("#patients3 ul[rel=sketches] span[rel="+data.id+"] .text").html($("#patients .title").val());
		//$("#protocol2_inactive").html($.nl2br($("#protocol2").val()));
		switch(data.access) {
			case "0":
				$("#patients3 ul[rel=sketches] span[rel="+data.id+"] .module-access-status").removeClass("module-access-active");
			break;
			case "1":
				$("#patients3 ul[rel=sketches] span[rel="+data.id+"] .module-access-status").addClass("module-access-active");
			break;
		}
		/*var treatid = data.id;
		if(data.changeSketchStatus != "0" && module.askStatus && $('.jqibox').length == 0) {
			...
			$.prompt(txt,{ 
				...
				submit: function(e,v,m,f){		
					if(v){
						$.ajax({ type: "GET", url: "/", dataType:  'json', data: 'path=apps/patients/modules/sketches&request=updateStatus&id=' + treatid + '&date=&status='+data.changeSketchStatus, cache: false, success: function(data){
							switch(data.status) {
								case "2":
									$("#patients3 ul[rel=sketches] span[rel="+treatid+"] .module-item-status").addClass("module-item-active").removeClass("module-item-active-stopped");
								...
								break;
								case "3":
									$("#patients3 ul[rel=sketches] span[rel="+treatid+"] .module-item-status").addClass("module-item-active-stopped").removeClass("module-item-active");
								break;
								default:
									$("#patients3 ul[rel=sketches] span[rel="+treatid+"] .module-item-status").removeClass("module-item-active").removeClass("module-item-active-stopped");
							}
	...
	this.statusOnClose = function(dp) {
		...
		$.ajax({ type: "GET", url: "/", dataType:  'json', data: "path=apps/patients/modules/sketches&request=updateStatus&id=" + id + "&date=" + date + "&status=" + status, cache: false, success: function(data){
				switch(data.status) {
					case "2":
						$("#patients3 ul[rel=sketches] span[rel="+data.id+"] .module-item-status").addClass("module-item-active").removeClass("module-item-active-stopped");
					break;
					case "3":
						$("#patients3 ul[rel=sketches] span[rel="+data.id+"] .module-item-status").addClass("module-item-active-stopped").removeClass("module-item-active");
					break;
					default:
						$("#patients3 ul[rel=sketches] span[rel="+data.id+"] .module-item-status").removeClass("module-item-active").removeClass("module-item-active-stopped");
				}
				if(data.changePatientStatus != 0) { module.setPatientStatus(data.changePatientStatus); }
				var pid = $("#patients").data("second");
				$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/invoices&request=getList&id="+pid, success: function(data){																																																																				
	...
	this.getDetails = function(moduleidx,liindex,list) {
		...
		if(self.coPrintOptions == '') {
			$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=getPrintOptions", success: function(html){
				self.coPrintOptions = html;
			}});
		}
		if(self.coSendToOptions == '') {
			$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=getSendToOptions", success: function(html){
				self.coSendToOptions = html;
			}});
		}
		if(self.coNewOptions == '') {
			$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=getNewOptions", success: function(html){
		...
		$.ajax({ type: "GET", url: "/", dataType:  'json', data: "path=apps/patients/modules/sketches&request=getDetails&id="+id, success: function(data){
			...
			if($('#checkedOut').length > 0) {
					$("#patients3 ul[rel=sketches] .active-link .icon-checked-out").addClass('icon-checked-out-active');
				} else {
					$("#patients3 ul[rel=sketches] .active-link .icon-checked-out").removeClass('icon-checked-out-active');
				}
	...
	/*this.actionNew = function() {
		...
		var id = $('#patients').data('second');
		$.ajax({ type: "GET", url: "/", dataType: 'json', data: 'path=apps/patients/modules/sketches&request=createNew&id=' + id, cache: false, success: function(data){
			$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=getList&id="+id, success: function(list){
				$("#patients3 ul[rel=sketches]").html(list.html);
				$('#patients_sketches_items').html(list.items);
				var liindex = $("#patients3 ul[rel=sketches] .module-click").index($("#patients3 ul[rel=sketches] .module-click[rel='"+data.id+"']"));
	...
	this.actionNew = function() {
		...
		copopup
			...
			.position({
				  my: "center center",
				  at: "right+123 center",
				  of: '#patientsActions .listNew',
				  collision: 'flip fit',
				  within: '#patients-right',
	...
	this.actionNewOption = function(option) {
		...
		switch(option) {
			case '1': // Drawing only
				...
				module.checkIn(cid);
				var id = $('#patients').data('second');
				$.ajax({ type: "GET", url: "/", dataType: 'json', data: 'path=apps/patients/modules/sketches&request=createNew&id=' + id + '&type=0&image=', cache: false, success: function(data){
					$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=getList&id="+id, success: function(list){
						$("#patients3 ul[rel=sketches]").html(list.html);
						$('#patients_sketches_items').html(list.items);
						var liindex = $("#patients3 ul[rel=sketches] .module-click").index($("#patients3 ul[rel=sketches] .module-click[rel='"+data.id+"']"));
			...
			case '2': // Drawing only
				...
				module.checkIn(cid);
				var id = $('#patients').data('second');
				$.ajax({ type: "GET", url: "/", dataType: 'json', data: 'path=apps/patients/modules/sketches&request=createNew&id=' + id + '&type=1&image=', cache: false, success: function(data){
					$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=getList&id="+id, success: function(list){
						$("#patients3 ul[rel=sketches]").html(list.html);
						$('#patients_sketches_items').html(list.items);
						var liindex = $("#patients3 ul[rel=sketches] .module-click").index($("#patients3 ul[rel=sketches] .module-click[rel='"+data.id+"']"));
			...
			case '3':
				...
				module.checkIn(cid);
				var id = $('#patients').data('second');
				$.ajax({ type: "GET", url: "/", data: 'path=apps/patients/modules/sketches&request=createNewImage&id=' + id, cache: false, success: function(html){
						$("#patients-right").empty().html(html);
						initPatientsContentScrollbar();	
					}
				});
			break;
	...
	this.actionDuplicate = function() {
		...
		var id = $("#patients").data("third");
		var pid = $("#patients").data("second");
		$.ajax({ type: "GET", url: "/", data: 'path=apps/patients/modules/sketches&request=createDuplicate&id=' + id, cache: false, success: function(mid){
			$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=getList&id="+pid, success: function(data){																																																																				
				$("#patients3 ul[rel=sketches]").html(data.html);
				$('#patients_sketches_items').html(data.items);
				var moduleidx = $("#patients3 ul").index($("#patients3 ul[rel=sketches]"));
				var liindex = $("#patients3 ul[rel=sketches] .module-click").index($("#patients3 ul[rel=sketches] .module-click[rel='"+mid+"']"));
				module.getDetails(moduleidx,liindex);
				$("#patients3 ul[rel=sketches] .module-click:eq("+liindex+")").addClass('active-link');
				}
			});
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					var id = $("#patients").data("third");
					var pid = $("#patients").data("second");
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=binSketch&id=" + id, cache: false, success: function(data){
							if(data == "true") {
								$.ajax({ type: "GET", url: "/", dataType:  'json', data: "path=apps/patients/modules/sketches&request=getList&id="+pid, success: function(data){
									$("#patients3 ul[rel=sketches]").html(data.html);
									$('#patients_sketches_items').html(data.items);
	...
	this.actionLoadTab = function(what) {
		$('#PatientsSketchesTabsContent > div:visible').hide();
		$('#'+what).show();
	...
	this.checkIn = function(id) {
		$.ajax({ type: "GET", url: "/", async: false, data: 'path=apps/patients/modules/sketches&request=checkinSketch&id='+id, success: function(data){
			if(!data) {
	...
	this.actionRefresh = function() {
		...
		var id = $("#patients").data("third");
		var pid = $("#patients").data("second");
		$("#patients3 ul[rel=sketches] .active-link").trigger("click");
		$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=getList&id="+pid, success: function(data){																																																																				
			$("#patients3 ul[rel=sketches]").html(data.html);
			$('#patients_sketches_items').html(data.items);
			var liindex = $("#patients3 ul[rel=sketches] .module-click").index($("#patients3 ul[rel=sketches] .module-click[rel='"+id+"']"));
			$("#patients3 ul[rel=sketches] .module-click:eq("+liindex+")").addClass('active-link');
			}
		});
	}
	...
	/*this.actionPrint = function() {
		var id = $("#patients").data("third");
		var url ='/?path=apps/patients/modules/sketches&request=printDetails&id='+id;
		if(!iOS()) {
	...
	this.actionPrint = function(option) {
		var id = $("#patients").data("third");
		var url ='/?path=apps/patients/modules/sketches&request=printDetails&option=plan&id='+id;
		if(!iOS()) {
			$("#documentloader").attr('src', url);
		} else {
			window.open(url);
	...
	this.actionSend = function() {
		var id = $("#patients").data("third");
		$.ajax({ type: "GET", url: "/", dataType:  'json', data: "path=apps/patients/modules/sketches&request=getSend&id="+id, success: function(data){
	...
	this.actionSendtoResponse = function() {
		var id = $("#patients").data("third");
		$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=getSendtoDetails&id="+id, success: function(html){
			$("#patientssketch_sendto").html(html);
	...
	this.sortclick = function (obj,sortcur,sortnew) {
		...
		$.ajax({ type: "GET", url: "/", dataType:  'json', data: "path=apps/patients/modules/sketches&request=getList&id="+fid+"&sort="+sortnew, success: function(data){
			$("#patients3 ul[rel=sketches]").html(data.html);
			$('#patients_sketches_items').html(data.items);
			obj.attr("rel",sortnew);
			obj.removeClass("sort"+sortcur).addClass("sort"+sortnew);
			var id = $("#patients3 ul[rel=sketches] .module-click:eq(0)").attr("rel");
			$('#patients').data('third',id);
			if(id == undefined) {
				return false;
			}
			...
			$("#patients3 ul[rel=sketches] .module-click:eq(0)").addClass('active-link');
		}
		});
	}
	...
	this.sortdrag = function (order) {
		...
		$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=setOrder&"+order+"&id="+fid, success: function(html){
	...
	this.actionDialog = function(offset,request,field,append,title,sql) {
		switch(request) {
			...
			case "getSketchStatusDialog":
				$.ajax({ type: "GET", url: "/", data: 'path=apps/patients/modules/sketches&request='+request+'&field='+field+'&append='+append+'&title='+title+'&sql='+sql, success: function(html){
					$("#modalDialog").html(html);
					$("#modalDialog").dialog('option', 'position', offset);
					$("#modalDialog").dialog('option', 'title', title);
					$("#modalDialog").dialog('open');
	...
	this.showItemContext = function(ele,uid,field) {
		...
		//var id = $("#"+field).val();
		if(ele.attr('edit') == 1) {
		$.ajax({ type: "GET", url: "/", data: 'path=apps/patients/modules/sketches&request=getTaskContext&id='+uid+'&field='+field, success: function(html){
	...
	this.insertStatus = function(rel,text) {
		var module = this;
		var html = '<div class="listmember" field="patientssketch_status" uid="'+rel+'" style="float: left">' + text + '</div>';
		$("#patientssketch_status").html(html);
		$("#modalDialog").dialog("close");
		$("#patientssketch_status").next().val("");
	...
	this.insertStatusDate = function(rel,text) {
		...
		$("#patientssketch_status").html(html);
		$("#modalDialog").dialog("close");
		$("#patientssketch_status").nextAll('img').trigger('click');
	}
	...
	this.insertFromDialog = function(field,gid,title) {
		...
		$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=getSketchTypeMin&id=" + gid, success: function(html){
	...
	this.newItem = function() {
		...
		var num = $("#patientssketchtasks>div").length;
		$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=addTask&pid=" + pid + "&mid=" + mid + "&num=" + num + "&sort=" + num, success: function(html){
			$('#patientssketchtasks').append(html);
			var idx = parseInt($('#patientssketchtasks .cbx').size() -1);
			//console.log(idx);
	...
	this.binItem = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
				$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=deleteTask&id=" + id, success: function(data){
					if(data){
	...
	this.newDrawing = function() {
		...
		var mid = $("#patients").data("third");
		zIndexes++;
		var curnum = $('#patients .canvasDrawSketch').size();
		var curcol = curnum % 10;
		var num = curnum+1;
		$.ajax({ type: "GET", url: "/", dataType: 'json', data: "path=apps/patients/modules/sketches&request=addDiagnose&mid=" + mid + "&num=" + num, success: function(data){
	...
	this.saveDrawing = function(id,img) {
		...
		var module = this;
		$.ajax({ type: "POST", url: "/", data: "path=apps/patients/modules/sketches&request=saveDrawing&id=" + id + "&img=" + imgsave, success: function(id){
			}
		});		
	}
	...
	this.actionHelp = function() {
		var url = "/?path=apps/patients/modules/sketches&request=getHelp";
		if(!iOS()) {
			$("#documentloader").attr('src', url);
		} else {
			window.open(url);
	...
	this.binDelete = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=deleteSketch&id=" + id, cache: false, success: function(data){
						if(data == "true") {
							$('#sketch_'+id).slideUp();
						}
					}
	...
	this.binRestore = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=restoreSketch&id=" + id, cache: false, success: function(data){
						if(data == "true") {
							$('#sketch_'+id).slideUp();
						}
					}
	...
	this.binDeleteItem = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=deleteSketchTask&id=" + id, cache: false, success: function(data){
						if(data == "true") {
	...
	this.binRestoreItem = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=restoreSketchTask&id=" + id, cache: false, success: function(data){
						if(data == "true") {
							$('#sketch_task_'+id).slideUp();
						}
					}
...
this.binDeleteColumn = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=deleteSketchDiagnose&id=" + id, cache: false, success: function(data){
						if(data == "true") {
	...
	this.binRestoreColumn = function(id) {
		...
		$.prompt(txt,{ 
			...
			submit: function(e,v,m,f){		
				if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=restoreSketchDiagnose&id=" + id, cache: false, success: function(data){
						if(data == "true") {
							$('#sketch_diag_'+id).slideUp();
						}
					}
	...
	this.manageCheckpoint = function(action,date) {
		...
		switch(action) {
			case 'new':
				$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=newCheckpoint&id=" + pid + "&date=" + date, cache: false });
			break;
			case 'update':
				$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=updateCheckpoint&id=" + pid + "&date=" + date, cache: false });			
			break;
			case 'delete':
				$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=deleteCheckpoint&id=" + pid, cache: false });
			break;
		}
	}
	...
	this.saveCheckpointText = function() {
		var pid = $('#patients').data('third');
		var text = $('#patients_sketchesCheckpoint textarea').val();
		$.ajax({ type: "POST", url: "/", data: "path=apps/patients/modules/sketches&request=updateCheckpointText&id=" + pid + "&text=" + text, cache: false });
	}
	
	/*this.calculateTasks = function() {
		var total = 0;
		var num = $('#PatientsSketchesThird .answers-outer-dynamic').size()*10;
		$('#PatientsSketchesThird .answers-outer-dynamic span').each( function() {
...
var patients_sketches = new patientsSketches('patients_sketches');
...
var colors = ['#6EAAFF','#FF7878','#FFD41D','#8CD264','#D2B4FF','#9999FF','#FF9E1F','#BCBCBC','#64CCC9','#FF83FF'];
	...
	function sketchesSelect(field,str,value) {
		...
		var costs = str[1];
		var minutes = str[2];
		var html = '<span class="listmember-outer"><a href="patients_sketches" class="showItemContext" edit="1" uid="' + id + '" field="'+field+'" costs="'+costs+'" minutes="'+minutes+'">' + value + '</a>';
		var app = getCurrentApp();
		var obj = getCurrentModule();
			if($("#"+field).html() != "") {
			...
			$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=saveLastUsedSketches&id="+id}); 
	...
	$(".sketches_image_uploader").livequery(function() {
		...
		patients_sketches.createUploader($(this));
	})
	...
	$('.sketches-search').livequery(function() { 
		$(this).autocomplete({
			appendTo: '#tabs-1',
			source: "?path=apps/patients/modules/sketches&request=getSketchesSearch",
			//minLength: 2,
		...
		$('div.loadCanvas.active').livequery( function() {
			$(this).draggable({
				...
				stop: function(e,ui){
					...
					$.ajax({ type: "GET", url: "/", dataType:  'json', data: "path=apps/patients/modules/sketches&request=updatePosition&id="+id+"&x="+x+"&y="+y, success: function(data){
						}
					});
				}
			});
		...
		$(document).on('click','span.clearTool',function(e) {
			...
			//var rel = $('#'+id).attr('rel');
			//patients_sketches.saveDrawing(rel,'');
			...
			restorePoint[id] = '';
			var rel = $('#'+id).attr('rel');
			patients_sketches.saveDrawing(rel,img);
		})
		...
		$(document).on('click','span.rotateTool',function(e) {
			...
			$.ajax({ type: "GET", url: "/CO_INC/classes/imagerotate.php", data: "image=" + image, success: function(data){
				if(data){
					patients_sketches.actionRefresh();
				} 
				}
			});
		})
	...
	$(document).on('click','a.binDiagnose',function(e) {
		...
		if($(this).hasClass('deactivated')) {
			...
			$.prompt(txt,{ 
				...
				submit: function(e,v,m,f){		
					if(v){
					$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=binDiagnose&id=" + id, success: function(data){
						if(data){
	...
	$(document).on('click', 'a.insertSketchfromDialog', function(e) {
		...
		var html = '<span class="listmember-outer"><a href="patients_sketches" class="showItemContext" edit="1" uid="' + cid + '" field="'+field+'" costs="'+costs+'" minutes="'+minutes+'">' + name + '</a>';
		var app = getCurrentApp();
		var obj = getCurrentModule();
			if($("#"+field).html() != "") {
				$("#"+field+" .showItemContext:visible:last").append(", ");
			...
			$('#'+app+' .coform').ajaxSubmit(obj.poformOptions);
			// save to lastused
			$.ajax({ type: "GET", url: "/", data: "path=apps/patients/modules/sketches&request=saveLastUsedSketches&id="+cid});
	});