...
class PatientsDocuments extends Patients {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/patients/modules/$name/";
			$this->model = new PatientsDocumentsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["PATIENT_DOCUMENT_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["PATIENT_PRINT_DOCUMENT"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("patients_documents",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["PATIENT_DOCUMENT_HELP"];
		$data["app"] = "patients";
...
$patientsDocuments = new PatientsDocuments("patientsDocuments");
?>