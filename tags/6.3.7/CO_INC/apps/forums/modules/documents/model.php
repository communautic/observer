...
class ForumsDocumentsModel extends ForumsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("forums-documents-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("forums-documents-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by title";
								$sortcur = '1';
						  } else {
		...
		$perm = $this->getForumAccess($id);
		$sql ="";
		if( $perm ==  "guest") {
			$sql = " and access = '1' ";
		}
	  
		$q = "select id,title,access,edited_date from " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where pid = '$id' and bin != '1' " . $sql . $order;
		$this->setSortStatus("forums-documents-sort-status",$sortcur,$id);
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function getDetails($id) {
		...
		$q = "SELECT * FROM " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$perms = $this->getForumAccess($array["pid"]);
		$array["canedit"] = false;
		$array["perms"] = $perms;
		...
		$doc = array();
		$qt = "SELECT * FROM " . CO_TBL_FORUMS_DOCUMENTS . " where did = '$id' and bin='0' ORDER BY created_date DESC";
		...
		$sendto = $this->getSendtoDetails("forums_documents",$id);
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " set title = '" . $lang["FORUM_DOCUMENT_NEW"] . "', pid='$id', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		if ($result) {
			return $id;
	...
	function createDuplicate($id) {
		...
		$q = "INSERT INTO " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " (pid,title,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"]. "'),'$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		
		// documents
		$q = "SELECT filename,tempname,filesize FROM " . CO_TBL_FORUMS_DOCUMENTS . " where did='$id' and bin='0'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			...
			$qt = "INSERT INTO " . CO_TBL_FORUMS_DOCUMENTS . " set did='$id_new', filename='$fsave', filesize='$filesize'";
			$resultt = mysql_query($qt, $this->_db->connection);
		}
   ...
   function binDocument($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreDocument($id) {
		$q = "UPDATE " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
	...
	function deleteDocument($id) {
		$q = "SELECT id FROM " . CO_TBL_FORUMS_DOCUMENTS . " where did = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$fid = $row["id"];
			$this->deleteFile($fid);
		...
		$q = "DELETE FROM co_log_sendto WHERE what='forums_documents' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function binDocItem($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_FORUMS_DOCUMENTS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
	...
	/*function getBin($id) {
		$arr = "";
		$qd ="select id, title, bin, bintime, binuser from " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where pid = '$id'";
	...
	function restoreFile($id) {
		$q = "UPDATE " . CO_TBL_FORUMS_DOCUMENTS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
	...
	function deleteFile($id) {
		$q = "SELECT filename,tempname FROM " . CO_TBL_FORUMS_DOCUMENTS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$q = "DELETE FROM " . CO_TBL_FORUMS_DOCUMENTS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getDocumentsDialog($request,$field,$append,$title,$sql,$id) {
		...
		$array["field"] = $field;
		$q = "select id, title from " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where pid='$id' and bin = '0' order by title";
		$result = mysql_query($q, $this->_db->connection);
	  	while ($row = mysql_fetch_array($result)) {
			foreach($row as $key => $val) {
				$array[$key] = $val;
   ...
   function getDocListFromIDs($string,$field){
		...
		foreach ($string as &$value) {
			$q = "SELECT id, title FROM " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where id = '$value' and bin='0'";
			$result = mysql_query($q, $this->_db->connection);
		...
		foreach ($arr as $key => &$value) {
			$users .= '<span class="docitems-outer"><a href="forums_documents" class="showItemContext" uid="' . $key . '" field="' . $field . '">' . $value;		
			if($i < $arr_total) {
				$users .= ', ';
			}
			$users .= '</a></span>';	
	...
	function getDocContext($id,$field){
		$q = "SELECT id,title FROM " . CO_TBL_FORUMS_DOCUMENTS_FOLDERS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		$row = mysql_fetch_array($result);
		foreach($row as $key => $val) {
			$array[$key] = $val;
	...
	function downloadDocument($id) {
		$q = "SELECT * FROM " . CO_TBL_FORUMS_DOCUMENTS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
