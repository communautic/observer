...
class TrainingsMeetings extends Trainings {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/trainings/modules/$name/";
			$this->model = new TrainingsMeetingsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["TRAINING_MEETING_ACTION_NEW"];
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("trainings_meetings",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["TRAINING_MEETING_HELP"];
		$data["app"] = "trainings";
	...
	function getTrainingsDialog() {
		global $system, $lang;
		$trainings = $this->model->getLast10Trainings();
	...
	function copyMeeting($pid,$phid) {
		global $lang;
		$training = $this->model->copyMeeting($pid,$phid);
