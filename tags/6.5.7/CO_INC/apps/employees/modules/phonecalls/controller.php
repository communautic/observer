...
class EmployeesPhonecalls extends Employees {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/employees/modules/$name/";
			$this->model = new EmployeesPhonecallsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["EMPLOYEE_PHONECALL_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["EMPLOYEE_PRINT_PHONECALL"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("employees_phonecalls",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["EMPLOYEE_PHONECALL_HELP"];
		$data["app"] = "employees";
