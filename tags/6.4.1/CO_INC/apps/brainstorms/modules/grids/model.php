...
class BrainstormsGridsModel extends BrainstormsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("brainstorms-grids-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("brainstorms-grids-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by title";
								$sortcur = '1';
						  } else {
		...
		$q = "select id,title,access,checked_out,checked_out_user from " . CO_TBL_BRAINSTORMS_GRIDS . " where pid = '$id' and bin != '1' " . $sql . $order;
		$this->setSortStatus("brainstorms-grids-sort-status",$sortcur,$id);
	...
	function checkoutGrid($id) {
		...
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_BRAINSTORMS_GRIDS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinGrid($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_BRAINSTORMS_GRIDS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getDetails($id) {
		...
		$ql = "SELECT * FROM co_brainstorms_grids_log where rid = '$id' ORDER BY created_date DESC";
   ...
   function saveGridColumns($cols) {
		foreach($cols as $key => $val) {
			$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS_COLUMNS . " set sort = '$key' WHERE id='$val'";
			$result = mysql_query($q, $this->_db->connection);
		}
		return "true";
   }
   ...
   function binGridColumn($id) {
		global $session;
		$now = gmdate("Y-m-d H:i:s");
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS_COLUMNS . " set bin = '1', bintime = '$now', binuser= '$session->uid' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return "true";
		}
   ...
   function getGridNote($id) {
	   global $session, $contactsmodel, $lang;
		...
		$q = "SELECT * FROM " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " where id='$id'";		
		$result = mysql_query($q, $this->_db->connection);
   ...
   function saveGridNewNote($pid,$id) {
		...
		$q = "INSERT INTO " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " set pid = '$pid', title = '$title', text = '$text', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNote($id,$title,$text) {
		global $session;
		$now = gmdate("Y-m-d H:i:s");
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " set title = '$title', text = '$text', edited_user = '$session->uid', edited_date = '$now' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_BRAINSTORMS_GRIDS . " set title = '" . $lang["BRAINSTORM_GRID_NEW"] . "', pid = '$id', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   ...
   function binGrid($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreGrid($id) {
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteGrid($id) {
		$q = "DELETE FROM " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " WHERE pid = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_BRAINSTORMS_GRIDS_COLUMNS . " WHERE pid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		/*$q = "DELETE FROM co_log_sendto WHERE what='grids' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);*/
		
		$q = "DELETE FROM " . CO_TBL_BRAINSTORMS_GRIDS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreGridColumn($id) {
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS_COLUMNS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteGridColumn($id) {
		...
		$q = "DELETE FROM " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " WHERE cid = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		/*$q = "DELETE FROM co_log_sendto WHERE what='grids' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);*/
		
		$q = "DELETE FROM " . CO_TBL_BRAINSTORMS_GRIDS_COLUMNS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function addTask($mid,$num,$sort) {
		...
		$q = "INSERT INTO " . CO_TBL_BRAINSTORMS_GRIDS_TASKS . " set mid='$mid', status = '0', title = '" . $lang["BRAINSTORM_GRID_TASK_NEW"] . "', sort='$sort'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		
		$task = array();
		$q = "SELECT * FROM " . CO_TBL_BRAINSTORMS_GRIDS_TASKS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			foreach($row as $key => $val) {
				$tasks[$key] = $val;
   ...
   function binItem($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " set bin = '1', bintime = NOW(), binuser= '$session->uid' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function deleteGridTask($id) {
		global $session;
		$q = "DELETE FROM " . CO_TBL_BRAINSTORMS_GRIDS_NOTES . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function convertToProject($id,$kickoff,$folder,$protocol) {
		...
		// copy all access 
		$qa = "SELECT * FROM co_brainstorms_access where pid='$brainstorm_id'";
		...
		$q = "INSERT INTO co_brainstorms_grids_log set rid = '$id', pid = '$pid', fid = '$folder', created_user = '$session->uid', created_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
