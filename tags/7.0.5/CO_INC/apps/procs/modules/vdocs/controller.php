...
class ProcsVDocs extends Procs {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/procs/modules/$name/";
			$this->model = new ProcsVDocsModel();
	...
	function getList($id,$sort,$fid=0) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["PROC_VDOC_ACTION_NEW"];
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("procs_vdocs",$id,$to,$subject,$body);
...
$procsVDocs = new ProcsVDocs("vdocs");
?>