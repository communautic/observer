...
class ProcsVDocsModel extends ProcsModel {
	...
	function getList($id,$sort,$fid=0) {
	  ...
	  if($sort == 0) {
		  ...
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("procs-vdocs-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by title";
								$sortcur = '1';
						  } else {
	  ...
	  $perm = $this->getProcAccess($id);
		...
		//$q = "select title,id,intern,startdate,enddate from " . CO_TBL_PROCS_PHASES . " where pid = '$id' and bin != '1' " . $order;
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_PROCS_VDOCS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkoutVDocs($id) {
		...
		$q = "UPDATE " . CO_TBL_PROCS_VDOCS . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinVDocs($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_PROCS_VDOCS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_PROCS_VDOCS . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
   ...
   function setDetails($pid,$id,$title,$content,$vdoc_access,$vdoc_access_orig) {
		...
		$q = "UPDATE " . CO_TBL_PROCS_VDOCS . " set title = '$title', content = '$content', access='$vdoc_access', $accesssql edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_PROCS_VDOCS . " set title = '" . $lang["PROC_VDOC_NEW"] . "', pid = '$id', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   	...
   	function createDuplicate($id) {
		global $session, $lang;
		// vdoc
		$q = "INSERT INTO " . CO_TBL_PROCS_VDOCS . " (pid,title,content) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"] . "'),content FROM " . CO_TBL_PROCS_VDOCS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function binVDoc($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PROCS_VDOCS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreVDoc($id) {
		$q = "UPDATE " . CO_TBL_PROCS_VDOCS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteVDoc($id) {
		...
		$q = "DELETE FROM co_log_sendto WHERE what='procs_vdocs' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_PROCS_VDOCS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_PROCS_VDOCS . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
