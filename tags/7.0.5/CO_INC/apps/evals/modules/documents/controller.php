...
class EvalsDocuments extends Evals {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/evals/modules/$name/";
			$this->model = new EvalsDocumentsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["EVAL_DOCUMENT_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["EVAL_PRINT_DOCUMENT"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("evals_documents",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["EVAL_DOCUMENT_HELP"];
		$data["app"] = "evals";
...
$evalsDocuments = new EvalsDocuments("evalsDocuments");
?>