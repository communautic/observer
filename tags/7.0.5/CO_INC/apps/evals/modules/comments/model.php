...
class EvalsCommentsModel extends EvalsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("evals-comments-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("evals-comments-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by item_date DESC";
								$sortcur = '1';
						  } else {
		...
		$q = "select id,title,item_date,access,status,checked_out,checked_out_user from " . CO_TBL_EVALS_COMMENTS . " where pid = '$id' and bin != '1' " . $sql . $order;
		$this->setSortStatus("evals-comments-sort-status",$sortcur,$id);
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_EVALS_COMMENTS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkoutComment($id) {
		...
		$q = "UPDATE " . CO_TBL_EVALS_COMMENTS . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinComment($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_EVALS_COMMENTS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_EVALS_COMMENTS . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getDetails($id, $option = "") {
		...
		$q = "SELECT * FROM " . CO_TBL_EVALS_COMMENTS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$sendto = $this->getSendtoDetails("evals_comments",$id);
   ...
   function setDetails($pid,$id,$title,$commentdate,$protocol,$management,$management_ct,$documents,$comment_access,$comment_access_orig) {
		...
		$q = "UPDATE " . CO_TBL_EVALS_COMMENTS . " set title = '$title', item_date = '$commentdate', protocol = '$protocol', management='$management', management_ct='$management_ct', documents = '$documents', access='$comment_access', $accesssql edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_EVALS_COMMENTS . " set title = '" . $lang["EVAL_COMMENT_NEW"] . "', item_date='$now', pid = '$id', status = '1', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   	...
   	function createDuplicate($id) {
		...
		// comment
		$q = "INSERT INTO " . CO_TBL_EVALS_COMMENTS . " (pid,title,item_date,start,end,protocol,management,management_ct,status,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"] . "'),item_date,start,end,protocol,management,management_ct,status,'$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_EVALS_COMMENTS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function binComment($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_EVALS_COMMENTS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreComment($id) {
		$q = "UPDATE " . CO_TBL_EVALS_COMMENTS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteComment($id) {
		...
		$q = "DELETE FROM co_log_sendto WHERE what='evals_comments' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_EVALS_COMMENTS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_EVALS_COMMENTS . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
