...
class ProcsDocuments extends Procs {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/procs/modules/$name/";
			$this->model = new ProcsDocumentsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["PROC_DOCUMENT_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["PROC_PRINT_DOCUMENT"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("procs_documents",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["PROC_DOCUMENT_HELP"];
		$data["app"] = "procs";
...
$procsDocuments = new ProcsDocuments("procsDocuments");
?>