...
class ProductionsDocuments extends Productions {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/productions/modules/$name/";
			$this->model = new ProductionsDocumentsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["PRODUCTION_DOCUMENT_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["PRODUCTION_PRINT_DOCUMENT"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("productions_documents",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["PRODUCTION_DOCUMENT_HELP"];
...
$productionsDocuments = new ProductionsDocuments("productionsDocuments");
?>