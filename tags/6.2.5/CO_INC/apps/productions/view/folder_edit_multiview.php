...
<div style="position: absolute; width: 100%; height: 30px; overflow: hidden">
    <div id="productionsFoldersSubTabs" class="contentSubTabs" style="position: absolute;">
	<ul>
...
<div  class="barchart-outer" style="position: relative; font-size: 11px; width: <?php echo($folder->css_width+225);?>px; height:<?php echo($folder->css_height+58);?>px">
    <div id="barchart-container-left" style="position: absolute; z-index: 5; width: 220px; padding-top: 37px; background-color:#FFF; height:<?php echo($folder->css_height+20);?>px">
        <div style="position: relative; padding-left: 10px; height: 16px; margin: 0 10px 2px 0;"><?php echo $lang['PRODUCTION_TIMELINE_ACTION'];?>
            <div style="text-align: center; position: absolute; width: 45px; padding: 1px 5px 0 0; top: 0; right: 0; height: 16px;"><?php echo $lang['PRODUCTION_TIMELINE_TIME'];?></div>
        </div>
        <?php 
		foreach($productions as $production){ ?>
		<div style="position: relative; height: 36px; margin: 0 10px 2px 0;">
        	<div style="position: absolute; padding-left: 10px; height: 16px; width: 150px; overflow: hidden; line-height: 16px; background-color: #e5e5e5;"><a class="but-scroll-to" t="<?php echo $production->css_top;?>" l="<?php echo $production->css_left;?>"><?php echo($production->title);?></a></div>
            <div style="text-align: right; position: absolute; width: 38px; padding: 0 10px 0 0; top: 0; right: 0; height: 16px; background-color: #e5e5e5;"><?php echo($production->days);?></div>
            
            <div style="position: absolute; top: 19px; padding-left: 10px; height: 16px; width: 100px; overflow: hidden; line-height: 16px; font-size: 9px; color: #666;"><a class="but-scroll-to" t="<?php echo $production->css_top;?>" l="<?php echo $production->css_left;?>"><?php echo($production->startdate);?>-<?php echo($production->enddate);?></a></div>
            <div style="text-align: right; position: absolute; top: 19px; width: 88px; padding: 1px 10px 0 0; right: 0; height: 16px; border-left: 2px solid #fff; font-size: 9px; color: #666;"><?php echo($production->name);?></div>
        </div>
      <?php 
	 } ?>
    </div>
	...
	?>
            ...
            <div style="position: relative; background-image:url(<?php echo($folder->bg_image);?>); background-position: <?php echo($folder->bg_image_shift);?>px 0px; width: <?php echo($folder->css_width);?>px; height:<?php echo($folder->css_height+8);?>px;">
				...
				if($production->kickoff_only) { ?>
					<div class="loadProduction" rel="<?php echo($production->id);?>" style="z-index: 2; position: absolute; top: <?php echo($production->css_top);?>px; left: <?php echo($production->css_left+$production->kickoff_space);?>px; height: 16px; width: 16px;"><img src="<?php echo CO_FILES;?>/img/kickoff.png" width="16" height="16" alt="" /></div>
				<?php } else {
				?>
                <div class="loadProduction <?php echo($production->status);?>" rel="<?php echo($production->id);?>" style="z-index: 2; position: absolute; top: <?php echo($production->css_top);?>px; left: <?php echo($production->css_left);?>px; height: 20px; width: <?php echo($production->css_width);?>px;"><span style="display: block; padding: 2px 0 0 5px;"><?php echo($production->realisation["real"]);?>%</span></div>
                <?php 
				}
				} ?>
            </div>
