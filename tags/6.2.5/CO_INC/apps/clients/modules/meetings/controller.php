...
class ClientsMeetings extends Clients {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/clients/modules/$name/";
			$this->model = new ClientsMeetingsModel();
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["CLIENT_PRINT_MEETING"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("clients_meetings",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["CLIENT_MEETING_HELP"];
		$data["app"] = "clients";
