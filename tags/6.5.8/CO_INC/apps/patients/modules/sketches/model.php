...
class PatientsSketchesModel extends PatientsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("patients-sketches-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("patients-sketches-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by title";
								$sortcur = '1';
						  } else {
		...
		$q = "select id,title,access,checked_out,checked_out_user from " . CO_TBL_PATIENTS_SKETCHES . " where pid = '$id' and bin != '1' " . $sql . $order;
		//echo $q;
		$this->setSortStatus("patients-sketches-sort-status",$sortcur,$id);
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_PATIENTS_SKETCHES . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkoutSketch($id) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinSketch($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_PATIENTS_SKETCHES . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getDetails($id, $option = "") {
		...
		$q = "SELECT a.* FROM " . CO_TBL_PATIENTS_SKETCHES . "  as a where a.id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		// get the diagnoses
		$diagnose = array();
		$q = "SELECT * FROM " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " where mid = '$id' and bin='0' ORDER BY sort";
		$result = mysql_query($q, $this->_db->connection);
		...
		$sendto = $this->getSendtoDetails("patients_sketches",$id);
   ...
   function getSketchPatientID($id) {
		global $session, $lang;
	    $q = "SELECT a.cid FROM " . CO_TBL_PATIENTS . " as a, " . CO_TBL_PATIENTS_SKETCHES . " as b WHERE b.id='$id' and b.pid = a.id";
	    $result = mysql_query($q, $this->_db->connection);
   ...
   function setDetails($pid,$id,$title,$canvasList_id,$canvasList_text,$sketch_access,$sketch_access_orig) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set title = '$title', access='$sketch_access', $accesssql edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		// do existing diagnoses
		$canvasList_size = sizeof($canvasList_id);
		foreach ($canvasList_id as $key => $value) {
			$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " set text = '$canvasList_text[$key]' WHERE id='$canvasList_id[$key]'";
			$result = mysql_query($q, $this->_db->connection);
		}
   ...
   function getStatus($id) {
		$q = "SELECT status FROM " . CO_TBL_PATIENTS_SKETCHES . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		return mysql_result($result,0);
	}
   ...
   function updateStatus($id,$date,$status) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set status = '$status', status_date = '$date', invoice_date = '$date', payment_reminder='$payment_reminder', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function updateStatusPatient($id) {
		$q = "SELECT a.status FROM " . CO_TBL_PATIENTS . " as a, " . CO_TBL_PATIENTS_SKETCHES . " as b WHERE b.id = '$id' and b.pid = a.id";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkPatientFinished($id) {
		$q = "SELECT a.status,a.id FROM " . CO_TBL_PATIENTS . " as a, " . CO_TBL_PATIENTS_SKETCHES . " as b WHERE b.id = '$id' and b.pid = a.id";
		$result = mysql_query($q, $this->_db->connection);
		$row = mysql_fetch_row($result);
		$patientstatus = $row[0];
		$pid = $row[1];
		
		$q = "SELECT status from " . CO_TBL_PATIENTS_SKETCHES . " WHERE pid = '$pid' and status !='2' and bin = '0';";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function createNew($id,$type,$image) {
		...
		$q = "INSERT INTO " . CO_TBL_PATIENTS_SKETCHES . " set title = '" . $lang["PATIENT_SKETCH_NEW"] . "', pid = '$id', type='$type', type_image='$image', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   	...
   	function createDuplicate($id) {
		...
		// sketch
		$q = "INSERT INTO " . CO_TBL_PATIENTS_SKETCHES . " (pid,title,type,type_image,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"] . "'),type,type_image,'$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_PATIENTS_SKETCHES . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function binSketch($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function restoreSketch($id) {
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function deleteSketch($id) {
		$q = "SELECT id FROM " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " WHERE mid = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$tid = $row["id"];
		...
		$q = "DELETE FROM co_log_sendto WHERE what='patients_sketches' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_USERS_CHECKPOINTS . " WHERE app = 'patients' and module = 'sketches' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "SELECT type_image FROM " . CO_TBL_PATIENTS_SKETCHES . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$bg = $row["type_image"];
		...
		$q = "DELETE FROM " . CO_TBL_PATIENTS_SKETCHES . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
	...
	function newCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "INSERT INTO " . CO_TBL_USERS_CHECKPOINTS . " SET uid = '$session->uid', date = '$date', app = 'patients', module = 'sketches', app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
 	...
 	function updateCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET date = '$date', status='0' WHERE uid = '$session->uid' and app = 'patients' and module = 'sketches' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
 	...
 	function deleteCheckpoint($id){
		global $session;
		$q = "DELETE FROM " . CO_TBL_USERS_CHECKPOINTS . " WHERE uid = '$session->uid'and app = 'patients' and module = 'sketches' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function updateCheckpointText($id,$text){
		global $session;
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET note = '$text' WHERE uid = '$session->uid' and app = 'patients' and module = 'sketches' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
    ...
    function getCheckpointDetails($id){
		global $lang;
		$row = "";
		$q = "SELECT a.pid,a.title,b.folder FROM " . CO_TBL_PATIENTS_SKETCHES . " as a, " . CO_TBL_PATIENTS . " as b WHERE a.pid = b.id and a.id='$id' and a.bin='0'";
		$result = mysql_query($q, $this->_db->connection);
		$row = mysql_fetch_array($result);
		if(mysql_num_rows($result) > 0) {
			$row['checkpoint_app_name'] = $lang["PATIENT_SKETCH_TITLE"];
			$row['app_id'] = $row['pid'];
			$row['app_id_app'] = $id;
		}
		return $row;
   ...
   function updatePosition($id,$x,$y){
		global $session;
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " SET xy = '".$x."x".$y."' WHERE id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
  ...
  function addDiagnose($mid,$num) {
		...
		$q = "SELECT numofDrawings FROM " . CO_TBL_PATIENTS_SKETCHES . " WHERE  id='$mid'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$now = gmdate("Y-m-d H:i:s");
		$y = $num*30;
		$q = "INSERT INTO " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " set mid='$mid', xy='30x$y', sort='$num', color='$numofDrawings'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		$colorreturn = $numofDrawings;
		$numofDrawings++;
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES . " set numofDrawings='$numofDrawings' WHERE id='$mid'";
		$result = mysql_query($q, $this->_db->connection);
  ...
  function binDiagnose($id) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);	
		return true;
   }
   ...
   function deleteSketchDiagnose($id) {
		global $session;
		$q = "DELETE FROM " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
    ...
    function restoreSketchDiagnose($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " set bin = '0' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function saveDrawing($id,$img) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_SKETCHES_DIAGNOSES . " set canvas='$img' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);	
		return true;
   }
