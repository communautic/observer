...
class ProcsMeetingsModel extends ProcsModel {
	  ...
	  if($sort == 0) {
		  ...
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("procs-meetings-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by item_date DESC";
								$sortcur = '1';
						  } else {
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_PROCS_MEETINGS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkoutMeeting($id) {
		...
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinMeeting($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_PROCS_MEETINGS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_PROCS_MEETINGS . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getDetails($id,$fid=0,$option = "") {
		...
		if($option = 'prepareSendTo') {
			$array["sendtoTeam"] = $this->_contactsmodel->checkUserListEmail($array["participants"],'procsparticipants', "", $array["canedit"]);
			$array["sendtoTeamNoEmail"] = $this->_contactsmodel->checkUserListEmail($array["participants"],'procsparticipants', "", $array["canedit"], 0);
			$array["sendtoError"] = false;
		}
		$array["participants"] = $this->_contactsmodel->getUserList($array['participants'],'procsparticipants', "", $array["canedit"]);
		...
		$array["management"] = $this->_contactsmodel->getUserList($array['management'],'procsmanagement', "", $array["canedit"]);
		...
		$q = "SELECT date,note FROM " . CO_TBL_USERS_CHECKPOINTS . " where uid='$session->uid' and app = 'procs' and module = 'meetings' and app_id = '$id' LIMIT 1";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function setDetails($pid,$id,$title,$meetingdate,$start,$end,$location,$location_ct,$participants,$participants_ct,$management,$management_ct,$task_id,$task_title,$task_text,$task,$task_sort,$documents,$meeting_access,$meeting_access_orig) {
		...
		foreach ($task_id as $key => $value) {
			...
			$q = "UPDATE " . CO_TBL_PROCS_MEETINGS_TASKS . " set status = '$checked_items[$key]', title = '$task_title[$key]', text = '$task_text[$key]' WHERE id='$task_id[$key]'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return $id;
   ...
   function updateStatus($id,$date,$status) {
		...
		$q = "SELECT title FROM " . CO_TBL_PROCS_MEETINGS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$title = mysql_result($result,0);
		
		$title_change = $title;
		if($status == 3) {
			$title_change = $title . " " . $lang["PROC_MEETING_POSPONED"];
		}
		
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS . " set title = '$title_change', status = '$status', status_date = '$date', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		if($status == 3) {
			...
			$q = "INSERT INTO " . CO_TBL_PROCS_MEETINGS . " (pid,title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,created_date,created_user,edited_date,edited_user) SELECT pid,'$title','$date',start,end,location,location_ct,length,management,management_ct,participants,participants_ct,0,'$now','$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_PROCS_MEETINGS . " where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
			if ($result) {
				$nid = mysql_insert_id();
				// do tasks
				$qt = "INSERT INTO " . CO_TBL_PROCS_MEETINGS_TASKS . " (mid,status,title,text,sort) SELECT '$nid',status,title,text,sort FROM " . CO_TBL_PROCS_MEETINGS_TASKS . " where mid='$id'";
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_PROCS_MEETINGS . " set title = '" . $lang["PROC_MEETING_NEW"] . "', item_date='$now', start='$time', end='$time', pid = '$id', participants = '$session->uid', management = '$session->uid', status = '0', status_date = '$now', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   	...
   	function createDuplicate($id) {
		...
		// meeting
		$q = "INSERT INTO " . CO_TBL_PROCS_MEETINGS . " (pid,title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status_date,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"] . "'),item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,'$now','$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_PROCS_MEETINGS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		// tasks
		$qt = "INSERT INTO " . CO_TBL_PROCS_MEETINGS_TASKS . " (mid,status,title,text,sort) SELECT $id_new,'0',title,text,sort FROM " . CO_TBL_PROCS_MEETINGS_TASKS . " where mid='$id' and bin='0'";
		$resultt = mysql_query($qt, $this->_db->connection);
		if ($result) {
			return $id_new;
		}
   ...
   function binMeeting($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreMeeting($id) {
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteMeeting($id) {
		$q = "SELECT id FROM " . CO_TBL_PROCS_MEETINGS_TASKS . " WHERE mid = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$tid = $row["id"];
		...
		$q = "DELETE FROM co_log_sendto WHERE what='procs_meetings' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_USERS_CHECKPOINTS . " WHERE app = 'procs' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_PROCS_MEETINGS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function addTask($mid,$num,$sort) {
		...
		$q = "INSERT INTO " . CO_TBL_PROCS_MEETINGS_TASKS . " set mid='$mid', status = '0', title = '" . $lang["PROC_MEETING_TASK_NEW"] . "', sort='$sort'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		
		$task = array();
		$q = "SELECT * FROM " . CO_TBL_PROCS_MEETINGS_TASKS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			foreach($row as $key => $val) {
				$tasks[$key] = $val;
	...
	function sortItems($task) {
		foreach($task as $key => $val) {
			$q = "UPDATE " . CO_TBL_PROCS_MEETINGS_TASKS . " set sort = '$key' WHERE id='$val'";
			$result = mysql_query($q, $this->_db->connection);
		}
		return "true";
   }
   ...
   function deleteTask($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS_TASKS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function restoreMeetingTask($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PROCS_MEETINGS_TASKS . " set bin = '0' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function deleteMeetingTask($id) {
		global $session;
		$q = "DELETE FROM " . CO_TBL_PROCS_MEETINGS_TASKS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
	...
	function newCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "INSERT INTO " . CO_TBL_USERS_CHECKPOINTS . " SET uid = '$session->uid', date = '$date', app = 'procs', module = 'meetings', app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
 	...
 	function updateCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET date = '$date', status='0' WHERE uid = '$session->uid' and app = 'procs' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
 	...
 	function deleteCheckpoint($id){
		global $session;
		$q = "DELETE FROM " . CO_TBL_USERS_CHECKPOINTS . " WHERE uid = '$session->uid'and app = 'procs' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function updateCheckpointText($id,$text){
		global $session;
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET note = '$text' WHERE uid = '$session->uid' and app = 'procs' and module = 'meetings' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
    ...
    function getCheckpointDetails($id){
		global $lang;
		$row = "";
		$q = "SELECT a.pid,a.title,b.folder FROM " . CO_TBL_PROCS_MEETINGS . " as a, " . CO_TBL_PROCS . " as b WHERE a.pid = b.id and a.id='$id' and a.bin='0'";
		$result = mysql_query($q, $this->_db->connection);
		$row = mysql_fetch_array($result);
		if(mysql_num_rows($result) > 0) {
			$row['checkpoint_app_name'] = $lang["PROC_MEETING_TITLE"];
			$row['app_id'] = $row['pid'];
			$row['app_id_app'] = $id;
		}
		return $row;
	...
	function copyMeeting($pid,$mid) {
		...
		$now = gmdate("Y-m-d H:i:s");
		// meeting
		$q = "INSERT INTO " . CO_TBL_PROCS_MEETINGS . " (pid,title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,documents,access,access_date,access_user,created_date,created_user,edited_date,edited_user) SELECT '$pid',title,item_date,start,end,location,location_ct,length,management,management_ct,participants,participants_ct,status,status_date,documents,access,access_date,access_user,'$now','$session->uid',edited_date,edited_user FROM " . CO_TBL_PROCS_MEETINGS . " where id='$mid'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		// tasks
		$qt = "INSERT INTO " . CO_TBL_PROCS_MEETINGS_TASKS . " (mid,status,title,text,sort) SELECT $id_new,status,title,text,sort FROM " . CO_TBL_PROCS_MEETINGS_TASKS . " where mid='$mid' and bin='0'";
		$resultt = mysql_query($qt, $this->_db->connection);
		// update copies of meeting
		$qc = "SELECT copies FROM " . CO_TBL_PROCS_MEETINGS . " where id='$mid'";
		...
		$copies .= $id_new;
		$qc = "UPDATE " . CO_TBL_PROCS_MEETINGS . " SET copies = '$copies' where id='$mid'";
		$resultc = mysql_query($qc, $this->_db->connection);
		
		if ($result) {
			$pro = array();
			$pro['title'] = $this->getProcTitle($pid);
			$pro['titlelink'] = $this->getProcTitleFromMeetingIDs(explode(",",$id_new),'meetings');
			return $pro;
		}
	}
