...
class ComplaintsVDocs extends Complaints {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/complaints/modules/$name/";
			$this->model = new ComplaintsVDocsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["COMPLAINT_VDOC_ACTION_NEW"];
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("complaints_vdocs",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["COMPLAINT_VDOC_HELP"];
		$data["app"] = "complaints";
...
$complaintsVDocs = new ComplaintsVDocs("vdocs");
?>