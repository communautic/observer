...
class ComplaintsModel extends Model {
   ...
   */   
   function getNumComplaints($id, $status="") {
	...
	function getComplaintTitleLinkFromIDs($array,$target){
		$total = sizeof($array);
		$data = '';
		if($total == 0) { 
			return $data; 
		...
		$i = 0;
		foreach ($array as &$value) {
			$q = "SELECT id,folder,title FROM " . CO_TBL_COMPLAINTS . " where id = '$value' and bin='0'";
			$result = mysql_query($q, $this->_db->connection);
			if(mysql_num_rows($result) > 0) {
				while($row = mysql_fetch_assoc($result)) {
					$arr[$i]["id"] = $row["id"];
		...
		$i = 1;
		foreach ($arr as $key => &$value) {
			$data .= '<a class="externalLoadThreeLevels" rel="' . $target. ','.$value["folder"].','.$value["id"].',1,complaints">' . $value["title"] . '</a>';
			if($i < $arr_total) {
				$data .= '<br />';
			}
			$data .= '';	
...
function getComplaintTitleFromMeetingIDs($array,$target, $link = 0){
		$total = sizeof($array);
		$data = '';
		if($total == 0) { 
			return $data; 
		...
		$i = 0;
		foreach ($array as &$value) {
			$qm = "SELECT pid,created_date FROM " . CO_TBL_COMPLAINTS_MEETINGS . " where id = '$value' and bin='0'";
			...
			if(mysql_num_rows($resultm) > 0) {
				...
				$q = "SELECT id,folder,title FROM " . CO_TBL_COMPLAINTS . " where id = '$pid' and bin='0'";
				$result = mysql_query($q, $this->_db->connection);
				if(mysql_num_rows($result) > 0) {
					while($row = mysql_fetch_assoc($result)) {
						$arr[$i]["id"] = $row["id"];
						$arr[$i]["item"] = $value;
						$arr[$i]["access"] = $this->getComplaintAccess($row["id"]);
						$arr[$i]["title"] = $row["title"];
						$arr[$i]["folder"] = $row["folder"];
						$arr[$i]["date"] = $date;
						$i++;
		...
		foreach ($arr as $key => &$value) {
			if($value["access"] == "" || $link == 0) {
				...
				$data .= '<a class="externalLoadThreeLevels" rel="' . $target. ','.$value["folder"].','.$value["id"].',' . $value["item"] . ',complaints">' . $value["title"] . '</a>';
			}
			if($i < $arr_total) {
				$data .= '<br />';
			}
   ...
   */
   ...
   function emptyBin() {
	...
	function getEditPerms($id) {
		global $session;
		$perms = array();
		$q = "SELECT a.pid FROM co_complaints_access as a, co_complaints as b WHERE a.pid=b.id and b.bin='0' and a.admins REGEXP '[[:<:]]" . $id . "[[:>:]]' ORDER by b.title ASC";
      	$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$perms[] = $row["pid"];
		}
   ...
   function getViewPerms($id) {
		global $session;
		$perms = array();
		$q = "SELECT a.pid FROM co_complaints_access as a, co_complaints as b WHERE a.pid=b.id and b.bin='0' and a.guests REGEXP '[[:<:]]" . $id. "[[:>:]]' ORDER by b.title ASC";
      	$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$perms[] = $row["pid"];
		}
   ...
   function setContactAccessDetails($id, $cid, $username, $password) {
	...
	function newCheckpoint($id,$date){
 	...
 	function updateCheckpoint($id,$date){
		global $session;
		$date = $this->_date->formatDate($date);
		$q = "UPDATE " . CO_TBL_USERS_CHECKPOINTS . " SET date = '$date', status='0' WHERE uid = '$session->uid' and app = 'complaints' and module = 'complaints' and app_id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function getGlobalSearch($term){
		...
		while($row = mysql_fetch_array($result)) {
			...
			if(in_array("documents",$active_modules)) {
				...
				// Documents
				$qp = "SELECT b.id,CONVERT(a.filename USING latin1) as title FROM " . CO_TBL_COMPLAINTS_DOCUMENTS . " as a, " . CO_TBL_COMPLAINTS_DOCUMENTS_FOLDERS . " as b WHERE b.pid = '$pid' and a.did = b.id and a.bin = '0' and b.bin = '0' and a.filename like '%$term%' ORDER BY a.filename";
				$resultp = mysql_query($qp, $this->_db->connection);
				while($rowp = mysql_fetch_array($resultp)) {
					$rows['value'] = htmlspecialchars_decode($rowp['title']);
					$rows['id'] = 'documents,' .$folder. ',' . $pid . ',' .$rowp['id'].',complaints';
	...
	function getComplaintsSearch($term,$exclude){
		...
		$q = "SELECT a.id,a.title as label FROM " . CO_TBL_COMPLAINTS . " as a WHERE a.id != '$exclude' and a.title like '%$term%' and  a.bin='0'" . $access ."ORDER BY a.title";
	...
	function getComplaintArray($string){
		$string = explode(",", $string);
		$total = sizeof($string);
		$items = '';
	...
	function getLast10Complaints() {
		global $session;
		$complaints = $this->getComplaintArray($this->getUserSetting("last-used-complaints"));
	  return $complaints;
	}
	...
	function saveLastUsedComplaints($id) {
		global $session;
		$string = $id . "," .$this->getUserSetting("last-used-complaints");
		$string = rtrim($string, ",");
		...
		$this->setUserSetting("last-used-complaints",$str);
	  return true;
	}
