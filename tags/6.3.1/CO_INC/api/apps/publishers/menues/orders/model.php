...
class API_PublishersMenuesOrdersModel {
	...
	function getList($sort) {
	  ...
	  if($sort == 0) {
		  ...
		  switch($sort) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("publishers-menues-sort-order",$id);
				  		if(!$sortorder) {
						  	$order = "order by item_date_from DESC";
								$sortcur = '1';
						  } else {
		...
		$q = "select id,title,item_date_from,item_date_to,access,status,checked_out,checked_out_user from " . CO_TBL_PUBLISHERS_MENUES . " where bin != '1' " . $sql . $order;
		...
		if($array["perms"] == "sysadmin" || $array["perms"] == "admin") {
			...
			if($array["checked_out"] == 1) {
				if($array["checked_out_user"] == $session->uid) {
					...
					$array["canedit"] = $this->checkoutMenue($id);
					$array["canedit"] = true;
				} else {
					$array["canedit"] = false;
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_PUBLISHERS_MENUES . " set title = '" . $lang["PUBLISHER_MENUE_NEW"] . "', item_date='$now', start='$time', end='$time', pid = '$id', status = '1', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
