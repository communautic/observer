...
class EmployeesDocumentsModel extends EmployeesModel {
   ...
   function setDetails($id,$title,$document_access) {
		...
		$q = "UPDATE " . CO_TBL_EMPLOYEES_DOCUMENTS_FOLDERS . " set title = '$title', access='$document_access', access_date='$access_date', access_user = '$session->uid', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function createNew($id) {
	 ...
	 function createDuplicate($id) {
			...
			// documents ???
			$qt = "INSERT INTO " . CO_TBL_EMPLOYEES_DOCUMENTS . " (did,filename,tempname,filesize) SELECT $id_new,filename,tempname,filesize FROM " . CO_TBL_EMPLOYEES_DOCUMENTS . " where did='$id' and bin='0'";
			$resultt = mysql_query($qt, $this->_db->connection);
   ...
   function restoreDocument($id) {
	...
	function deleteFile($id) {
		$q = "SELECT tempname FROM " . CO_TBL_EMPLOYEES_DOCUMENTS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
