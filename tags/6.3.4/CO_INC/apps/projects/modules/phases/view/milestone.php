...
<table border="0" cellpadding="0" cellspacing="0" class="table-content tbl-inactive" style="margin-bottom: 0px;">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td class="tcell-right"><?php if($phase->canedit) { ?><input name="task_startdate[<?php echo $value->id;?>]" type="text" class="input-date datepicker task_start ms" value="<?php echo $value->startdate;?>" readonly="readonly" /><input name="task_movedate_start[<?php echo $value->id;?>]" type="hidden" value="<?php echo $value->startdate;?>" /><input name="task_enddate[<?php echo $value->id;?>]" type="hidden" value="" /><input name="task_movedate[<?php echo $value->id;?>]" type="hidden" value="<?php echo $value->startdate;?>" /><?php } else { ?><?php echo $value->startdate;?><?php } ?>
      </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      ...
      <td class="tcell-right"><input id="dependent<?php echo $value->id;?>" name="task_dependent[<?php echo $value->id;?>]" type="hidden" value="<?php echo $value->dependent;?>"><span id="dependent<?php echo $value->id;?>-text"><?php if($phase->canedit) { ?><a field="dependent<?php echo $value->id;?>" uid="<?php echo $value->id;?>" class="showItemContext" href="projects_phases"><?php } ?><?php echo $value->dependent_title;?><?php if($phase->canedit) { ?></a><?php } ?></span></td>
    </tr>
  </table>
