...
class EvalsMeetings extends Evals {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/evals/modules/$name/";
			$this->model = new EvalsMeetingsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["EVAL_MEETING_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["EVAL_PRINT_MEETING"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("evals_meetings",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["EVAL_MEETING_HELP"];
		$data["app"] = "evals";
	...
	function getEvalsDialog() {
		global $system, $lang;
		$evals = $this->model->getLast10Evals();
	...
	function copyMeeting($pid,$phid) {
		global $lang;
		$eval = $this->model->copyMeeting($pid,$phid);
