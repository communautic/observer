...
class ForumsDocuments extends Forums {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/forums/modules/$name/";
			$this->model = new ForumsDocumentsModel();
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["FORUM_PRINT_DOCUMENT"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("forums_documents",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["FORUM_DOCUMENT_HELP"];
		$data["app"] = "forums";
...
$forumsDocuments = new ForumsDocuments("forumsDocuments");
?>