...
class EmployeesDocuments extends Employees {
	var $module;

	function __construct($name) {
			$this->module = $name;
			$this->form_url = "apps/employees/modules/$name/";
			$this->model = new EmployeesDocumentsModel();
	...
	function getList($id,$sort) {
		...
		$data["sort"] = $arr["sort"];
		$data["perm"] = $arr["perm"];
		$data["title"] = $lang["EMPLOYEE_DOCUMENT_ACTION_NEW"];
	...
	function printDetails($id,$t) {
		...
		$GLOBALS['SECTION'] = $session->userlang . "/" . $lang["EMPLOYEE_PRINT_DOCUMENT"];
		switch($t) {
			case "html":
	...
	function sendDetails($id,$variable,$to,$cc,$subject,$body) {
		...
		// write sento log
		$this->writeSendtoLog("employees_documents",$id,$to,$subject,$body);
	...
	function getHelp() {
		global $lang;
		$data["file"] =  $lang["EMPLOYEE_DOCUMENT_HELP"];
		$data["app"] = "employees";
...
$employeesDocuments = new EmployeesDocuments("employeesDocuments");
?>