...
class PatientsPhonecallsModel extends PatientsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("patients-phonecalls-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("patients-phonecalls-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by item_date DESC";
								$sortcur = '1';
						  } else {
		...
		$q = "select id,title,item_date,access,status,checked_out,checked_out_user from " . CO_TBL_PATIENTS_PHONECALLS . " where pid = '$id' and bin != '1' " . $sql . $order;
		$this->setSortStatus("patients-phonecalls-sort-status",$sortcur,$id);
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_PATIENTS_PHONECALLS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkoutPhonecall($id) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_PHONECALLS . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinPhonecall($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_PATIENTS_PHONECALLS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_PATIENTS_PHONECALLS . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getDetails($id, $option = "") {
		...
		$q = "SELECT * FROM " . CO_TBL_PATIENTS_PHONECALLS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		switch($array["status"]) {
			case "0":
				$array["status_text"] = $lang["PATIENT_PHONECALL_STATUS_OUTGOING"];
				$array["status_date"] = '';
			break;
			case "1":
				$array["status_text"] = $lang["PATIENT_PHONECALL_STATUS_ON_INCOMING"];
				$array["status_date"] = '';
			break;
		}
		
		$sendto = $this->getSendtoDetails("patients_phonecalls",$id);
   ...
   function setDetails($pid,$id,$title,$phonecalldate,$start,$end,$protocol,$management,$management_ct,$documents,$phonecall_access,$phonecall_access_orig,$phonecall_status,$phonecall_status_date) {
		...
		$q = "UPDATE " . CO_TBL_PATIENTS_PHONECALLS . " set title = '$title', item_date = '$phonecalldate', start = '$start', end = '$end', protocol = '$protocol', management='$management', management_ct='$management_ct', documents = '$documents', access='$phonecall_access', $accesssql status = '$phonecall_status', status_date = '$phonecall_status_date', edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_PATIENTS_PHONECALLS . " set title = '" . $lang["PATIENT_PHONECALL_NEW"] . "', item_date='$now', start='$time', end='$time', pid = '$id', status = '1', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
   	...
   	function createDuplicate($id) {
		...
		// phonecall
		$q = "INSERT INTO " . CO_TBL_PATIENTS_PHONECALLS . " (pid,title,item_date,start,end,protocol,management,management_ct,status,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"] . "'),item_date,start,end,protocol,management,management_ct,status,'$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_PATIENTS_PHONECALLS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function binPhonecall($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_PATIENTS_PHONECALLS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restorePhonecall($id) {
		$q = "UPDATE " . CO_TBL_PATIENTS_PHONECALLS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deletePhonecall($id) {
		...
		$q = "DELETE FROM co_log_sendto WHERE what='patients_phonecalls' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_PATIENTS_PHONECALLS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_PATIENTS_PHONECALLS . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
