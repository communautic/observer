...
class ComplaintsGridsModel extends ComplaintsModel {
	...
	function getList($id,$sort) {
		global $session;
	  if($sort == 0) {
		  $sortstatus = $this->getSortStatus("complaints-grids-sort-status",$id);
		  if(!$sortstatus) {
			  ...
			  switch($sortstatus) {
				  ...
				  break;
				  case "3":
				  		$sortorder = $this->getSortOrder("complaints-grids-sort-order",$id);
				  		if(!$sortorder) {
								$order = "order by title";
								$sortcur = '1';
						  } else {
		...
		$q = "select id,title,access,checked_out,checked_out_user from " . CO_TBL_COMPLAINTS_GRIDS . " where pid = '$id' and bin != '1' " . $sql . $order;
		$this->setSortStatus("complaints-grids-sort-status",$sortcur,$id);
	...
	function checkoutGrid($id) {
		...
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS . " set checked_out = '1', checked_out_user = '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
	...
	function getNavNumItems($id) {
		...
		$q = "select count(*) as items from " . CO_TBL_COMPLAINTS_GRIDS . " where pid = '$id' and bin != '1' " . $sql;
		$result = mysql_query($q, $this->_db->connection);
	...
	function checkinGrid($id) {
		...
		$q = "SELECT checked_out_user FROM " . CO_TBL_COMPLAINTS_GRIDS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$user = mysql_result($result,0);

		if($user == $session->uid) {
			$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS . " set checked_out = '0', checked_out_user = '0' where id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		if ($result) {
			return true;
	...
	function getDetails($id) {
		...
		$q = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		...
		$array["owner"] = $contactsmodel->getUserList($array['owner'],'complaintgridowner', "", $array["canedit"]);
		...
		$array["management"] = $contactsmodel->getUserList($array['management'],'complaintgridmanagement', "", $array["canedit"]);
		...
		$array["team"] = $contactsmodel->getUserList($array['team'],'complaintgridteam', "", $array["canedit"]);
		...
		$cols = array();
		$num_notes = array('0');
		$q = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " where pid = '$id' and bin='0' ORDER BY sort";
		$result = mysql_query($q, $this->_db->connection);
		$days = 0;
		while($row = mysql_fetch_object($result)) {
			...
			//$qn = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " where cid = '$colID' and bin='0' ORDER BY istitle DESC,isstagegate ASC,sort ASC";
			$qn = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " where cid = '$colID' and bin='0' ORDER BY sort";
		...
		$console_items = array();
		$pid = $array["pid"];
		$qc = "SELECT * FROM " . CO_TBL_COMPLAINTS_NOTES . " where pid = '$pid' and bin='0' ORDER BY title ASC";
		...
		$sendto = $this->getSendtoDetails("complaints_grids",$id);
		...
		$ql = "SELECT * FROM co_complaints_grids_log where rid = '$id' ORDER BY created_date DESC";
		...
		$arr = array("grid" => $grid, "cols" => $cols, "colheight" => $colheight, "listheight" => $listheight, "sendto" => $sendto, "access" => $array["perms"], "projects" => $projects);
		return $arr;
   }
   ...
   function setDetails($pid,$id,$title,$owner,$owner_ct,$management,$management_ct,$team,$team_ct,$grid_access,$grid_access_orig) {
		...
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS . " set title = '$title', owner = '$owner', owner_ct = '$owner_ct', management = '$management', management_ct = '$management_ct', team = '$team', team_ct = '$team_ct', $accesssql edited_user = '$session->uid', edited_date = '$now' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
   ...
   function saveGridColumns($cols) {
		foreach($cols as $key => $val) {
			$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " set sort = '$key' WHERE id='$val'";
			$result = mysql_query($q, $this->_db->connection);
		}
		return "true";
   }
   ...
   function saveGridColDays($id,$days) {
			$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " set days = '$days' WHERE id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		return "true";
   }

   function newGridColumn($id,$sort) {
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " set pid = '$id',sort = '$sort'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		if ($result) {
			return $id;
   ...
   function binGridColumn($id) {
		global $session;
		$now = gmdate("Y-m-d H:i:s");
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " set bin = '1', bintime = '$now', binuser= '$session->uid' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return "true";
		}
   ...
   function saveGridItems($col,$items) {
		...
		foreach($items as $key => $id) {
			$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set istitle = '0', isstagegate = '0', cid = '$col', sort = '$key' WHERE id='$id'";
			$result = mysql_query($q, $this->_db->connection);
		}
		return "true";
   }
   ...
   function getGridNote($id) {
	   global $session, $contactsmodel, $lang;
		...
		$q = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " where id='$id'";		
		$result = mysql_query($q, $this->_db->connection);
   ...
   function saveGridNewNote($pid,$id) {
		global $session, $contactsmodel, $lang;
		$now = gmdate("Y-m-d H:i:s");
		$q = "SELECT title,text FROM " . CO_TBL_COMPLAINTS_NOTES . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$row = mysql_fetch_row($result);
		$title = mysql_real_escape_string($row[0]);
		$text = mysql_real_escape_string($row[1]);
		
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set pid = '$pid', title = '$title', text = '$text', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNewNoteTitle($pid,$id,$col) {
		...
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set pid = '$pid', cid = '$col', istitle = '1', isstagegate = '0', title = '$title', text = '$text', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNoteTitle($id,$col) {
		...
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set cid = '$col', istitle = '1', isstagegate = '0', edited_user = '$session->uid', edited_date = '$now' WHERE id = '$id'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return $id;
		}
   ...
   function saveGridNewNoteStagegate($pid,$id,$col) {
		...
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set pid = '$pid', cid = '$col', istitle = '0', isstagegate = '1', title = '$title', text = '$text', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNoteStagegate($id,$col) {
		...
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set cid = '$col', istitle = '0', isstagegate = '1', edited_user = '$session->uid', edited_date = '$now' WHERE id = '$id'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return $id;
		}
   ...
   function saveGridNewManualNote($pid) {
		global $session, $lang;
		$now = gmdate("Y-m-d H:i:s");
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set pid = '$pid', title='" . $lang["COMPLAINT_GRID_ITEM_NEW"]. "', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNewManualTitle($pid,$col) {
		global $session, $lang;
		$now = gmdate("Y-m-d H:i:s");
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set pid = '$pid', cid='$col', istitle = '1', title='" . $lang["COMPLAINT_GRID_TITLE_NEW"]. "', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNewManualStagegate($pid,$col) {
		global $session, $lang;
		$now = gmdate("Y-m-d H:i:s");
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set pid = '$pid', cid='$col', isstagegate = '1', title='" . $lang["COMPLAINT_GRID_STAGEGATE_NEW"]. "', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";		
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			$id = mysql_insert_id();
			return $id;
   ...
   function saveGridNote($id,$title,$text) {
		global $session;
		$now = gmdate("Y-m-d H:i:s");
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set title = '$title', text = '$text', edited_user = '$session->uid', edited_date = '$now' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	  ...
	  /*function toggleMilestone($id,$ms) {
		global $session;
		$now = gmdate("Y-m-d H:i:s");
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set ms = '$ms', edited_user = '$session->uid', edited_date = '$now' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
			return true;
		}
	...
	function createNew($id) {
		...
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS . " set title = '" . $lang["COMPLAINT_GRID_NEW"] . "', pid = '$id', created_user = '$session->uid', created_date = '$now', edited_user = '$session->uid', edited_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
	...
	function createDuplicate($id) {
		...
		// grid
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS . " (pid,title,owner,owner_ct,management,management_ct,team,team_ct,created_date,created_user,edited_date,edited_user) SELECT pid,CONCAT(title,' " . $lang["GLOBAL_DUPLICAT"] . "'),owner,owner_ct,management,management_ct,team,team_ct,'$now','$session->uid','$now','$session->uid' FROM " . CO_TBL_COMPLAINTS_GRIDS . " where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		$id_new = mysql_insert_id();
		// cols
		
		$q = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " WHERE pid = '$id' and bin='0'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			$colID = $row["id"];
			$sort = $row['sort'];
			$days = $row['days'];
			$qc = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " set pid = '$id_new', sort='$sort', days='$days'";
			$resultc = mysql_query($qc, $this->_db->connection);
   ...
   function binGrid($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS . " set bin = '1', bintime = NOW(), binuser= '$session->uid' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreGrid($id) {
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteGrid($id) {
		$q = "DELETE FROM " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " WHERE pid = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		$q = "DELETE FROM " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " WHERE pid='$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		/*$q = "DELETE FROM co_log_sendto WHERE what='grids' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);*/
		
		$q = "DELETE FROM " . CO_TBL_COMPLAINTS_GRIDS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function restoreGridColumn($id) {
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " set bin = '0' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function deleteGridColumn($id) {
		...
		$q = "DELETE FROM " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " WHERE cid = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		
		/*$q = "DELETE FROM co_log_sendto WHERE what='grids' and whatid='$id'";
		$result = mysql_query($q, $this->_db->connection);*/
		
		$q = "DELETE FROM " . CO_TBL_COMPLAINTS_GRIDS_COLUMNS . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function toggleIntern($id,$status) {
		global $session;
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS . " set intern = '$status' where id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if ($result) {
		  	return true;
		}
   ...
   function addTask($mid,$num,$sort) {
		...
		$q = "INSERT INTO " . CO_TBL_COMPLAINTS_GRIDS_TASKS . " set mid='$mid', status = '0', title = '" . $lang["COMPLAINT_GRID_TASK_NEW"] . "', sort='$sort'";
		$result = mysql_query($q, $this->_db->connection);
		$id = mysql_insert_id();
		
		$task = array();
		$q = "SELECT * FROM " . CO_TBL_COMPLAINTS_GRIDS_TASKS . " where id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		while($row = mysql_fetch_array($result)) {
			foreach($row as $key => $val) {
				$tasks[$key] = $val;
   ...
   function binItem($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set bin = '1', bintime = NOW(), binuser= '$session->uid' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   	...
   	function setItemStatus($id,$status) {
		...
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set status = '$status' WHERE id = '$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function restoreGridTask($id) {
		global $session;
		$q = "UPDATE " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " set bin = '0' WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function deleteGridTask($id) {
		global $session;
		$q = "DELETE FROM " . CO_TBL_COMPLAINTS_GRIDS_NOTES . " WHERE id='$id'";
		$result = mysql_query($q, $this->_db->connection);
		if($result) {
			return true;
		}
   ...
   function convertToProject($id,$kickoff,$folder,$protocol) {
		...
		$q = "INSERT INTO co_complaints_grids_log set rid = '$id', pid = '$pid', fid = '$folder', created_user = '$session->uid', created_date = '$now'";
		$result = mysql_query($q, $this->_db->connection);
